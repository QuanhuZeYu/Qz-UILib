# 网络层方案：HTTP-like 内容语义双端通信

> **状态：现行规范**（网络层方案，内容语义版已落地）——Channel + Fetch + Stream + Store 四层 API 为现行能力，Realtime 子层仍是实验性。4.1LTS 是立项沿革，不作为现状依据；实现以 `net.api` / `net.core` / `net.codec` / `net.transport` 实时源码为准。
> 当前实现已从“每种请求一个 Java 类型”调整为 `route/key + contentType + headers + body` 的内容模型；POJO 反射 codec 仅作为可选辅助，不再是协议身份。
...
- 网络自检场景规格归档在 [历史报告/规划与设计/网络自检.md](../../历史报告/规划与设计/网络自检.md)（历史·演进记录，不构成现行要求）；当前 `/qzuilib test` 进入系统性重构期，旧 `net_self_check` 页面入口已清空，后续按 RemoteNet 分组恢复。

配套文档：

- [使用文档/02-控件/网络层入门.md](../../使用文档/02-控件/网络层入门.md)
- [网络编解码线格式.md](网络编解码线格式.md)
- [网络原版Mixin注入策略.md](网络原版Mixin注入策略.md)
- [历史报告/规划与设计/网络自检.md](../../历史报告/规划与设计/网络自检.md)
...
仅客户端的 `NetStoreUiBridge` 是 Store 视图到调用方渲染回调的主线程投递端口：`bind(NetStoreView, Object element, NetStoreRenderer)` 把 element 原样交回业务 renderer，框架不解释它的类型（旧 document 栈时期它是 `ElementNode`，该栈已删除）。它是网络侧向客户端 UI 投递 Store 快照的唯一端口，本身不依赖任何 UI 栈。
...
### 决策 7：GTNH 开发依赖基线兼容性约束

以项目当前 GTNH 开发依赖基线（`dependencies.gradle:38` 的 `setGtnhVersion`）为准，默认 vanilla mixin transport 可实现，但实现期必须遵守：
...
| `club.heiqi.uilib.internal.devtools` | 仅客户端 | 网络自检项注册与执行（入口状态见 [`网络自检.md`](../../历史报告/规划与设计/网络自检.md)） | ❌ |
...
<table name="关键文件">
  ...
  <row>
    ...
    <responsibility>仅客户端 Store → 业务渲染回调的主线程投递端口</responsibility>
  </row>
  <row>
    <path>`src/main/java/club/heiqi/uilib/internal/devtools/NetSelfCheckRegistry.java`、`NetSelfCheckRunner.java`、`NetRuntimeSelfChecks.java`</path>
    <responsibility>运行时自检项注册与执行</responsibility>
  </row>
</table>
...
- `NetStoreUiBridge`：Store renderer 投递到客户端主线程。
