...
> 状态：**现行规范**（文件类型为 UI/UX 设计方案；下述「现行部分」即本文件的现行内容，其余章节为就地保留的施工记录）
> 范围：`SceneDataTable` 编辑列（`Column.textInput` / `Column.select`）的视觉提示
> **现行部分**：编辑列必须一眼可辨——编辑槽用内凹 well/inset（暗底 + 1px 边框 + caret/下拉箭头），hover/聚焦/编辑三态以边框升亮与主题聚焦色区分，槽底不随斑马纹变化以形成列级「输入列带」。
> **历史部分**：§2 与 §8 的立项期色值、§9「给 fixer」的实现提示、§10 待拍板项都是施工期记录；其中 `ScenePalette` 与 `SceneStateColors` 已随全库液态玻璃迁移删除，`ROW_BG_EVEN`/`ROW_BG_ODD`/`BG_ENABLED` 等实色常量不再存在。
> 当前实现：`SceneDataTable` 默认复用已主题化的 `SceneTextInput` / `SceneSelect`，外观由 `SceneSurfaceBinder` 与 `SceneChromeTokens` / 主题 token 派生（见 `SceneDataTableEditorChrome` 类注释）；具体色值以主题与实时源码为准，不以本文数值为准。
...
### 2.1 立项期色值来源（`ScenePalette` 已删除，仅作历史对照）

| 常量 | 色值 | 来源 | 用途 |
|---|---|---|---|
| `ROW_BG_EVEN` | `0xFF1E293B` | ScenePalette.java（已删除） | 偶数行斑马纹底 |
| `ROW_BG_ODD` | `0xFF243B53` | ScenePalette.java（已删除） | 奇数行斑马纹底 |
...
- 新增常量集中放在 `SceneDataTable` 私有常量区（§2.2）；不再有 `ScenePalette` 需要避让（该类已随主题迁移删除）。
