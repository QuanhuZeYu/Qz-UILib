...
本文定义 `/qzuilib test` 重构后的页面体系、语义覆盖矩阵和运行时人工验收文本规范。当前旧 test 子页已清空，命令入口进入 P0 语义首页。
...
P0 首页先建立分组索引、运行时测试结果模型和统一用例卡片。首页只展示 DOM / CSS / Layout / Paint 分组入口，不恢复旧页面名。

| 首页卡片 | 覆盖语义 | 运行时预期文本 | P0 状态 |
|---|---|---|---|
| DOM-001 | `appendChild` 返回插入节点并移动已有节点 | 预期结果：点击执行后 A 节点移动到 B 节点后方，页面显示 `返回节点：A`。 | 未执行 |
| CSS-001 | inline style 高于样式表规则 | 预期结果：同一元素最终显示为 inline 指定颜色。 | 未执行 |
| LAYOUT-001 | block normal flow 垂直布局 | 预期结果：三块内容从上到下排列，垂直间距与标尺一致。 | 未执行 |
| PAINT-001 | background、border、text 绘制顺序 | 预期结果：背景在最底层，边框压住背景边缘，文本位于最上层。 | 未执行 |

P0 首页卡片必须完整显示 `用例编号`、`覆盖语义`、`自动断言`、`操作步骤`、`预期结果`、`实际结果`、`状态` 七个字段。后续每个分组页恢复运行时按钮前，仍必须先在本文对应分组表补齐编号、语义和 `预期结果：...` 文本。
