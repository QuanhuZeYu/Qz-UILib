...
- Section：active gesture 先到 UP/CANCEL，再 dismiss overlay 并立即执行严格单 live 切换；incoming 标题与字段 presentation shell 始终保持 `opacity=1`，完整布局发布后从 `-22px` 以 emphasized 240ms 归位，相邻项延迟 36ms、最大启动延迟 252ms。级联使用不改 LayoutBox 的像素对齐 presentation geometry offset，统一平移卡片内容与后代 TextInput/Select clip；父 viewport clip 保持固定，不得在动画期间裁掉控件上沿，也不得为每张卡片创建全屏 transform layer。旧 section Owner 立即销毁，快速反选不等待 outgoing completion，禁止大面积 `1→0→1` 明灭。
...
Motion 使用 host 每帧同一单调时间，不写 Draft/Persistence/history，不新建全局 ticker。stagger helper 保持 internal；不做 exit snapshot、layout animation、spring 或 Hero。不得在输入开放时移动交互子树；section presentation shell 位移期间必须关闭整棵子树的 hit-test 与 Tab traversal，逐项 offset 归零或 Owner 卸载后恢复原门禁并请求 hover 重算。
...
- 自动化固定 max width、footer 不滚动、error-over-dirty、现有保存语义、多 occurrence motion isolation、透明色 alpha-safe 插值、TextInput 背景稳定、字段 reveal 的 presentation offset/后代 clip 同步与零 transform layer、同帧/逐帧滚轮累计与 ease-out 轨迹、反向滚轮、scrollbar 中途接管、跨帧 hover 几何同步、主树/overlay 首帧布局发布、三轮 settle 上限，以及 reveal 期间 hit-test/Tab 门禁与 Owner 恢复。
