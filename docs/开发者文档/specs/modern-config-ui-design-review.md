...
**证据**（全部 `读源码`）：
- root padding=20（上下共 40）：ConfigScreen.java:216
- root gap=12：:217，分隔 titleBar/statusSummary/navBar/scrollContainer/actionBar 共约 4 个间隙 = 48
- titleBar 固定高 44：ConfigTheme.java:60
- statusSummary 固定高 34：ConfigTheme.java:62
- actionBar 固定高 46：ConfigTheme.java:64
- navBar（SceneSegmented）行高 ≈ 文本 16 + 2×PAD_LG(10) = 36（`推断`，SceneSegmented.java:121 padding + 默认字号）
...
**证据**：
- 三按钮顺序 append，row gap=10，无分区：ConfigScreen.java:451-458（`mountButton` 依次挂"恢复默认"/"取消更改"/"保存"）。
- 按钮固定宽 110：ConfigTheme.java:29。三按钮 + 2 gap = 350px，靠左。
...
**修复指引（给 fixer）**：`createActionBar`（:449）在"恢复默认"和"取消"之间插一个 `fillParentWidth` 的 spacer 空节点撑开；若 `SceneNode` 支持 `MainAxisAlign.SPACE_BETWEEN` 则用对齐属性。`SceneSegmented.java:16` import 了 `MainAxisAlign`，说明枚举存在，fixer 需确认是否有 `SPACE_BETWEEN` 成员。
...
**证据**：
- statusSummary 是 ROW，gap=10，依次 append：dirty 徽标（ConfigScreen.java:250）、error 徽标（:260）、feedback 文本（:287）。
...
**证据**：`createScrollContainer` gap=0（ConfigScreen.java:375）；viewport padding=14（:355）但 padding 是 viewport 内部，scrollbar 在 viewport 外侧紧贴。
...
**证据**：badge `padding=8`、`cornerRadius=999`、无 preferredHeight：ConfigScreen.java:528-529。
...
**问题描述**：titleBar 副行直接显示 "modId: " + schema.modId()（ConfigScreen.java:234），技术化文案面向最终用户不友好。

**证据**：ConfigScreen.java:234。设计 §8 已提"建议改为 schema 提供的人类可读标题"，并标"待拍板是否给 ConfigSchema 加 title 字段"（`modern-config-ui-design.md` 第 319 行）。属设计已知占位，非偏离。

### m3【小】viewport 宽度被 scrollbar 占 4px——实测影响极小，非阻断

**问题描述**：viewport flexGrow=1 占剩余宽，scrollbar 固定 4px，viewport 实际宽 = 容器宽 − 4。

**证据**：ConfigScreen.java:352（viewport flexGrow=1）、SceneScrollbar.java:137（column 固定 4px）。
...
| §5 NUMBER slider 读数 | slider 旁加数值读数 | ❌ **偏离/未落地** | NumberFieldRenderer.java:53-66 无 readout（见 M1） |
| §5 CHOICE 阈值 4 | ≤4 Segmented >4 Select | ✅ 落地 | ChoiceFieldRenderer.java:27/49 |
| §6 状态栏计数 | dirtyCount/errorCount 徽标 | ✅ 落地 | ConfigScreen.java:252/262，已接 dirtyCountSignal/errorCountSignal |
...
**颜色体系：成体系 ✅**
- config 层有完整语义色板：ROOT_BG/VIEWPORT_BG/CARD_BG 三档深蓝背景（ConfigTheme.java:32-36）、CARD_BORDER 三态（:38-42）、TITLE/TEXT/MUTED/ERROR/OK/DIRTY 文本色（:45-55）。
- 状态色语义清晰：dirty=蓝(0xFF60A5FA)、error=红(0xFFF87171)、ok=绿(0xFF34D399)，与 scene 控件层 `SceneChromeTokens.ACCENT` 蓝系协调（SceneChromeTokens.java:29-33）。
...
**间距体系：基本成体系 ✅**
- 复用 SceneChromeTokens 的 PAD/GAP/RADIUS 档位（ConfigTheme.java:19-23 委托）。
- 卡片 padding=PAD_LG(10)、gap=GAP_MD(8)、radius=RADIUS_LG(6)，档位一致。
- **瑕疵**：root padding=20、gap=12 是硬编码裸数字（ConfigScreen.java:216-217），没走 token 档位，与卡片间距体系脱节。建议归入 ConfigTheme 常量。
