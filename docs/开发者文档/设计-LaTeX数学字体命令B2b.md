...
状态：已完成现有能力与参考源码核查，生产实现等待本方案的公共 API 与可见行为裁定。基线提交 76c2a3d6；本文件不是命令已实现或视觉验证已通过的声明。
...
- 显式 mathrm/mathit 的数学字形按局部选择屏蔽外层富文本斜体再叠加；旧源码中未使用新命令的字形沿用原行为。该覆盖必须同时验证正常绘制与阴影。
...
批准以上 MathFontStyle、LatexAtom、LatexGroup 与 GlyphElem 构造器／只读接口增量；批准新命令使用现有斜切近似和局部字体选择优先于宿主斜体的行为。批准后直接实施上述回归、软件验证、完整构建和本地提交，不再为常规实现步骤重复确认。
