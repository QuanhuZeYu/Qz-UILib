...
**通用预防**：跨渲染边界入口必须用真实 GL 状态保存/恢复包裹完整生命周期，禁硬编码 `glEnable`/`glDisable` 模拟结束状态。
...
**通用预防**：圆角裁剪须区分"外观装饰"与"后代裁剪"两独立语义；脱流元素裁剪相对 containing block 而非 DOM 父级。
...
**通用预防**：字体异步管线须以 runtimeVersion 隔离代际；烘焙字号与渲染缩放分母须同一常量，禁独立系数分别缩放；同 API 的 trim/wrap/measure 重载须保持宽度口径一致。
...
**通用预防**：布局/样式/命中等只读流程绝不能改文档失效版本，布局期临时元素须走静默写入入口；结构标脏须区分"容器需重排子项"与"未变兄弟子树仍干净"。
...
**通用预防**：滚动偏移变化后须重新命中测试更新 hover；COMPOSITE 级 transform 平移的可命中节点须考虑 hit-test 只读 LayoutBox 契约，视觉位置偏离时由父节点转发交互。
...
**通用预防**：HUD 注册表遍历须基于快照或等效防御；多层 HUD 输入只路由最上层命中层；非交互菜单页不执行 HUD 输入探测或反射深扫。
...
**通用预防**：动画属性 clamp 须在归一化阶段完成，不能插值后跳变；opacity 路径切换须在帧边界而非动画中途。
...
**通用预防**：`GRADLE_USER_HOME` 须显式设置避免中文路径；依赖版本须锁定一致；并行构建须避免共享 build 目录。
...
**通用预防**：纯 JVM 测试不直接实例化继承 `GuiScreen`/`BaseScreen` 的页面类；可能触发 GL/LWJGL 的路径须有 headless 保护。
...
**通用预防**：配置保存须先验证再写入，失败回滚内存值；渲染上下文须完整传递所有运行时适配器；Forge 配置分类查询不能用隐式创建分类的 API 代替存在性检查。
...
**通用预防**：默认行为须在事件传播完成后检查 `isDefaultPrevented()` 再执行；GUI 打开须延迟到帧结束后；reactive 去重须基于「帧初值 vs 帧末终值」在 flush 阶段裁定，不在 set 时拿未 flush 旧值比较。
...
**通用预防**：早期 mixin 已拿到的生命周期对象应优先直接传递或缓存，不在构造期依赖全局单例反查；跨模组网络协议握手须等 FML connection-established 语义成立后再发送。
...
**通用预防**：fixer→build→test→review→commit 是硬性链路，改动简单不是跳过 review 的借口；per-frame 热路径日志禁用 INFO 级，诊断日志须走统一节流入口；决策升级（尤其包依赖方向）须先派 Oracle 做可行性评估，用户确认架构意图 ≠ 可行性已验证。
