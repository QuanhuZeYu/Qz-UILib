...
- 移除自动 keyframe opacity 后，紫色小圆角结束态下第一次点击 `Click target` 仍可能屏闪，因为点击 handler 仍把 opacity 从 `1.0` 改成 `0.55` 并触发 opacity transition。
...
- `Click target` 的点击 transition 曾同时覆盖 background-color、border-radius 与 opacity；第一次点击从 keyframe fill 末帧进入蓝色小圆角时，opacity transition 也会触发同一 FBO 路径切换。
...
- 点击后的第一下屏闪同理，不是重排主因；`Click target: 0 -> 1` 文案长度不变，核心差异是 opacity transition 让元素从普通绘制进入 opacity paint context。

## 修复方案

- 将 `smokePulse` 自动 keyframe 收口为纯 background-color + border-radius 三段 stop 探针。
- 将 `Click target` 点击 transition 也收口为 background-color + border-radius，不再改变 opacity。
- 页面底部独立 `Group opacity probe` 继续作为 opacity/group opacity 的独立验收入口。
- 测试中固定 `smokePulse` 不再注册 `OPACITY` 轨道，且 `Click target` 的 transition properties 不包含 `OPACITY`，避免后续再次把多段 keyframe/点击探针与 FBO 路径混在一起。
...
- 若后续仍需要验收 opacity transition，应放在独立可控探针里，并先修复/稳定 opacity paint context 的运行时路径。
