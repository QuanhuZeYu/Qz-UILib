...
真实入口深度，恢复时形成 `0 < 1` 的伪下溢。内置 Minecraft renderer 另有一层无条件
`glPushAttrib/glPopAttrib`，只降级外层仍会留下 GL error。问题与具体物品（包括 `minecraft:log`）无关。
...
- 内置 Minecraft renderer 复用 server attribute stack 的同一探测语义，不支持时不调用不可用 API，
  支持时保留原有内层保护，renderer 异常继续传播。
- 支持路径仍保留完整恢复验证；renderer 弹掉围栏栈帧时继续报告不可恢复并中止帧。

## 预防措施

固定管线栈必须逐子能力探测，不得由一个 stack 的结果推断另一个；每项都要把合法值域与紧邻查询的
GL error 一并作为能力门。能力降级应收窄到最小子围栏，不得关闭整个 guard，也不得吞掉 renderer
或帧中止异常。

## 验证与待验收

自动化覆盖 texture/server/client depth=0、查询错误并清理、depth>=1 完整保护、支持路径真实下溢，
以及内置 renderer 不支持时零 push/pop 且异常传播。
