...
- **截图读数与代码探针冲突时，先验产物的构建出处，再动结论**：采信任何一侧之前，先确认那张截图
  对应的 jar 是哪个提交、哪次构建的产物（文件时间戳、版本号、构建树 git 状态三查），再谈谁对谁错。
  2026-09-05 实案：实机截图与 headless 探针互相矛盾，因未先查截图出自哪个 jar，把一个**正确的**
  代码侧结论错误撤回，绕完整一圈才恢复。冲突的第三种解释永远在候选清单里：**产物过期**——
  它既不是「图读错了」也不是「探针撒谎了」，而是两端根本量的不是同一版码。
