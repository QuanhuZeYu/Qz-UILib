...
- GitHub Release 采用 `github-release-permission-split/v1`：仓库自有 `_github-release-contract.yml` 是纯 read reusable，仅接收 immutable annotated tag object、peeled commit 三项身份并负责 gate/preflight；`_github-release-publish.yml` 是最小 write wrapper，先以 read 调合同，再由唯一 write job消费同 run artifact。verify-only 只进入 read contract，tag/recovery publish 才进入 wrapper。
