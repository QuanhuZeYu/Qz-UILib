...
- portal 中“当前成员”最多 8 行，“搜索结果”最多 12 行；两区仍按实际内容动态收缩，超过各自 cap 后保持可滚动，整个 portal 继续受可用高度裁剪，不以无界列表撑高浮层。
...
- 列表成员管理由早期较小窗口调整为当前成员 8 行、搜索结果 12 行，以减少常见规则集的滚动；动态收缩、分区滚动和 portal 可用高度裁剪契约不变。
