...
### F36 markdown 页的几何/渲染分叉：一个被「字号 0」牵出来的既有缺陷（2026-09-18）
...
| 倍率 | commands | text | segments | bounds |
|---|---|---|---|---|
| 100 | 101 | 16 | 62 | 1279 x **751** |
| 150 | 101 | 16 | 62 | 1309 x 818 |
| 200 | 101 | 16 | 62 | **1592** x 885 |
| 0 | 24 | **0** | **0** | 1279 x **885** |
...
**根因**：`MarkdownPageContent.nodes()` 的几何取自 `node.getFontSize()`（**声明值**，该页恒为
`BASE_FONT_PX`）与 L2 命令坐标；而节点在场景里的实际布局与渲染走 `effectiveFontSize()`
（**生效值** = 声明值 × 用户倍率）。两者只在倍率 100% 时相等。审核者引的
`Math.max(1, baseFontSizePx)` 那几处**不是**元凶（那里的 `baseFontSizePx` 是 14，不是 0）。

**为什么不在本轮修**：几何要改成「用生效字号并随字号失效」，等于把段流盒的行高/宽度交给布局引擎按
`effectiveFontSize()` 测量 —— 而本页源码注释明写外观刻意不与主题/环境发生关系、并有 `MarkdownPageTest`
反向钉住与目检基线（G16 禁止项）。属独立批次，本轮只在 `MarkdownPageContent.nodes()` 留下现场注释。
