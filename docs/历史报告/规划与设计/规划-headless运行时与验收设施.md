...
| `--page-index=8` fs=100 偶发两种输出 | **成立 → 已定位并修复** | 复现：连跑 50 次得 48 次 `a8fb1156…`、2 次 `2669a01c…`（约 4%，与审核记录的一对 hash 相同）。根因：`TextLayoutService.tryAcquireWidthMissBudget()` 的宽度缓存 miss 预算按 **16ms 真实时间窗**限流，超预算的码点按空格宽近似排版 ⇒ 哪些码点被近似取决于本次运行的真实耗时，布局宽度随机器负载漂移。修复：headless 会话解除预算（`HeadlessSession.open` 置 `FontConfig.widthCacheMissBudgetPerWindow = 0`），连跑 50 次 = 50:1。生产侧「按时间窗降级」口径未改，是否改为与时间无关的降级量待决策 |
