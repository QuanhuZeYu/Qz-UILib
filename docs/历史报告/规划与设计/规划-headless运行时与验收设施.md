...
**顺带发现（未修，另批）**：headless 出图存在**跨进程非确定性**——同一二进制、同一命令行在不同时间窗
运行会得到 3 种像素形态（**同一时间窗内连续 5 次逐像素相同**）。证据：同代码跨窗差异 9590 px
（x=12..247、166 行），旧码 vs 新码 9984 px，而一对跨版本反而只差 1683 px ⇒ 差异由「运行批次」而非
代码版本决定，且集中在文字区；已排除落盘缓存（运行期无任何文件写出），疑似字形栅格化 / 字体回退的
跨进程顺序差异。**后果**：像素级对拍不能作为回归判据，出图验收须以命令面（commands / bounds / clip /
segments）+ 像素自检（`glError=0` / ink / opaque）为准；`colors` 随之波动（1225 / 1233 / 1242），
不是稳定判据。
