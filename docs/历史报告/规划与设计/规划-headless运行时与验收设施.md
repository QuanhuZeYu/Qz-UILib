...
- **M4 铺开**：**部分完成**（F18）——启动器（冷启动 1.62 s）、分辨率矩阵 4 档批量（2.42 s 跑完、4/4 ok）、
  GL 上下文进程级复用修复已完成；**剩余：消费者域扩展**（控件/表单/浮层/HUD/chat3 页面纳入 headless 可渲染面）、
  常驻进程形态（候选）。360P 外壳溢出已定根因并转业务待办。
