...
- **A2（回退）**：无 GL 环境（无 natives / 上下文创建失败）回退到现有软光栅出图（§六-6）。
...
- **M2 渲染地基**：**部分完成**（F14）——字体 GL 尾端已由「命令面 × 像素面」证据确认上屏（92 条文本命令 /
  2100 字符 → 像素 100% 覆盖、1290 色）；出图完整性判据已落地（命令有像素无 = 失败、几何全越界 = 失败、部分越界 = 提示）。
  **剩余：与既有软光栅出图对拍一次**——前置是需要一个可指定文本的诊断页面（text-probe）与软光栅侧的同文本渲染入口，
  排在下一轮首项。
