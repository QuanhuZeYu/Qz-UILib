...
> 状态：**历史规划记录**——路线 A（breaking 一步到位）已采纳并落地 H1/H2/H3（进展见 §5 状态列）；H4（HUD 输入）未开工，是投影化的唯一触发器（见 [投放职责聚合方案.md](../../开发者文档/规格文档/投放职责聚合方案.md)）。§1 的「今天的 HUD」与 §4 的两条兼容路线是立项时快照，不作为现状依据。
...
本规划直接解决《[2026-08 深度架构审查](../../历史报告/审查/2026-08-深度架构审查.md)》中的 A1「HUD 平行管线与特殊编码」条目；
