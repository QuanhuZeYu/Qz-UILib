...
> 状态：**历史·演进记录**（立项期实现蓝图，主体已落地）——三态（Authority / Draft / Persistence）与「渲染层只绑草稿」等契约已实现，现行契约以实时源码与 `docs/使用文档/v4.x-LTS-稳定API清单.md` 为准；本文的「已验证事实摘要」「五、实现顺序与依赖图」「六、软依赖与打包裁决」「七、验收检查清单」「八、裁决记录」是 2026-06-28 立项期的侦察与施工材料，只证明当时的设计依据，不构成现行要求。
> 基础：三态四层软依赖架构（旧决策已删除）
> 对齐宪章：`AGENTS.md` §设计取向与主权（原 NORTH_STAR 宪章已并入、不再单列文件，signal-first、分层隔离、帧末批处理等约束含义以该节与引用处注释为准）+ 反模式
> 本文档是实现蓝图，不含实现体。【已验证】= 读代码确认；【设计提议】= 本施工图提出的方案。

## 已验证事实摘要（2026-06-28 侦察结论，只作当时的立项前提；部分结论已被后续重构推翻，例如其中的 `ScenePalette` / `SceneStateColors` 已删除）
...
- 【已验证】reactive 层：`Signal.create/get/set`、`Computed.create(Supplier)/get/dispose`、`ReadableSignal<T>.get()`；写入经 `ReactiveScheduler` 帧末批处理。
...
- 守：决策"Schema 纯数据声明无 UI 依赖"；数据层不 import 渲染/样式层。
...
- 守：决策"游戏读取唯一来源"；不持 signal，数据层不 import 渲染/样式层。
...
- 守：写失败时 Authority/current 尚未提交；写入统一走帧末批处理。
...
5. 编辑
   用户输入 → SceneTextInput handler → onChange.accept(newValue)
     → adapter.onFieldEdit(path, v)
         → draftSignal(path).set(v)        // 经 ReactiveScheduler 帧末批处理
...
| 2 | 三态物理隔离 | 单测：DraftBuffer 与 Authority 内部 Map 不同实例；编辑 draft 后 authority.get 不变 | 决策"三态物理隔离"、数据层不 import 渲染/样式层 |
...
| 8 | UI 只绑草稿 | grep ConfigScreen/FieldRenderer 无 authority.applyAll；只经 adapter signal | 决策"渲染层只绑草稿"、handler 只发布 signal |
| 9 | signal-first | 编辑路径：handler 只调 onFieldEdit→signal.set，不直接 node.setText | signal-first（handler 只发布 signal） |
| 10 | 整页事务 | 单测：一次 save = 一次 writeAll | 帧末批处理，无中间态 |
