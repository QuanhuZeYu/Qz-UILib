...
> 状态：**历史·演进记录**（首版已于 2026-08-13 实施，提交 323d25da；缓存/批量/预乘待后续）。素材基线：09-原版物品渲染流程.md（2026-08-13）
>
> **后续演进（2026-08-17 后）**：图 1 的「纯 2D 图标 → Qz 2D 等价自绘」分支已不再成立——现行实现中纯 2D / 3D block / 多 pass 一律完整委托原版 `renderItemAndEffectIntoGUI`，并新增 `RenderSemantics`（VANILLA/ISOLATED）两语义与 `ItemRenderTierRegistry` 三级渲染分级；现行机制见 [架构图 08-物品渲染包装](../../开发者文档/架构图/08-物品渲染包装.md)。本文保留为首版演进记录。
