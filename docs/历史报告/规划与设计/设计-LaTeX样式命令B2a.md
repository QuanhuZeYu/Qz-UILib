# LaTeX 样式命令 B2a：已批准的接口方案

状态：用户已在提交 e76328d5 的具体方案说明后指示“继续”，按本方案批准实施。基于 B1 提交 d88bd9c0；实际验证结果随 B2a 交付记录。
...
- 矩阵单元进入既有 TEXT 基线，各单元内可显式覆盖，列与行边界结束局部声明。left/middle/right 的 middle 不应凭实现分段意外结束同一个定界数学列表的声明。既有各 part 的 LatexGroup 样式字段记录分段入口状态，子因子仍记录各自声明；下一段入口保存 middle 处样式，即便声明与 middle 之间没有因子也不丢失。middle 前 glue 取该入口，后 glue 取下一段首因子的有效样式；这不新增 AST 类型或访问接口。
...
本次批准涵盖上述新增枚举、基类只读字段接口、节点构造器重载与分式独立覆盖，以及这些显式命令从字面降级变成样式语义。直接实施、验证并本地提交 B2a，无需再次确认常规实现步骤。
