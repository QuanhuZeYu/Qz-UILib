...
本文适用于 4.8.0 起的当前实现（scene 栈），并登记其后各版本（含 5.0.0 登记面）的新增面；每一项以三种状态标记给出：**✅ 稳定**（业务代码可依赖，在本版本期内不随意变更签名或语义）、**⚠️ 公开但不稳定**（对外可见但不承诺稳定，如 beta 能力、引擎细节）、**🔒 内部**（不承诺稳定，不列公共面）。判定口径见「阅读约定」，具体适用范围以各条目登记与当前源码为准。
...
- **⚠️ 公开但不稳定**（施工期 / beta / 签名观察期，对外可见但不承诺稳定）——判定理由分三类：
  ...
  - **采样设施，明确不承诺**：`UiPerfMarkers`、`UiPerformanceMonitor.recordCounter(String, long)` / `MAX_SCREEN_HISTORIES`、`UiRuntimeStats.getCounterSummary()`——标记名与计数器是**诊断/采样面**（`Config.useDebug` 关闭时不建会话、不分配），不是运行时契约；其「新增公共 API 追认」本身在**工作站** `team/P0-验收记录.md` 中登记为「待 ADR 评审确认」，故不能标 ✅。
...
- **🔒 不列公共面**：`SceneNode.__structureVersion()`、`SceneRuntime.__setViewportLogicalBox(int,int)`（`__` 双下划线内部桥）；`PickerDensityPreferences` / `PickerDensityPreferenceSource` 属 MC 依赖桥接层（`uilib.config.modern`，本文既有 ⚠️ 口径），本版**不升格**为 LTS 稳定承诺——其是否转为 ✅ 待后续批次裁定（**工作站** `team/P6-终验记录.md` §10.6 登记）。
...
- **4.9.1 定档时的远端范围 = `[4.9.0,4.10.0)`**：4.9.0 拒绝旧端，旧端拒绝 4.9.0，双端需同步升级。该值其后已调整（提交 `9bb73003`，2026-09-12）：现值 `[4.9.0,4.11.0)`（见 `src/main/java/club/heiqi/uilib/MyMod.java:24`），上界抬到制品 `4.10.0` 的下一 minor 边界，覆盖 `4.9.x` 与 `4.10.x`。
