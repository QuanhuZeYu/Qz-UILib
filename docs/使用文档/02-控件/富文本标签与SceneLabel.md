...
- **现代软换行（word-break）**：拉丁等单词按词边界整词折行，CJK 单字任意字间可断；行尾空白折叠、
  行首空白丢弃；宽度超过整行的超长单词按字符硬断；ZWSP 提供词内软断行机会、
  软连字符断行处显示连字符、变体选择符/组合标记附着前字不落行首（见下「控制字符统一口径」）。
...
- **`<br>` 与裸换行符**：富文本模式下统一折叠为换行标记（零宽，不产生字形），
  裸 `\n`/`\r`/`\v`/`\f`/NEL(U+0085)/LS(U+2028)/PS(U+2029) 全部视作换行（`\r\n` 折叠为一个）。
...
所有渲染解析器（RAW / MINECRAFT / RICH 三模式、SceneLabel、文本域视觉行、tooltip 省略）
共享同一套 Unicode 控制字符口径（`font.util.UnicodeTextClassifier`，默认开启、无开关）：

| 类别 | 码点 | 行为 |
|---|---|---|
| 换行类 | `\n \r \v \f NEL(U+0085) LS(U+2028) PS(U+2029)` | 统一折叠为换行；`\r\n` 折叠为一个 |
| 制表符 | `\t` | 固定 4 个空格列宽（渲染为空格字形，advance 按 4×空格宽推进） |
| 可折叠空白 | 空格 / NBSP / U+1680 / U+2000..200A / U+202F / U+205F / 全角空格 | 断词分隔、行尾折叠、行首丢弃（统一语义） |
| 软断行 | ZWSP(U+200B) | 词内软断行机会（断行处零宽不显示） |
| 软连字符 | U+00AD | 行内零宽不可见；断行处显示连字符 `-` |
| 连字控制 | ZWNJ/ZWJ(U+200C/U+200D) | 零宽、跳过渲染（无连字布局引擎） |
| 变体选择符 | U+FE00..FE0F / U+E0100..E01EF | 零宽、附着前一字形、不落行首 |
| 组合标记 | Mn/Mc/Me | 与前字符合并成簇、不单独断行；多层组合标记逐层向上堆叠（金字塔） |
...
- **NFC 规范化（挡 1）**：显示/换行/裁剪/链接路径把「基字 + 组合标记」序列自动合并为预组合字符
