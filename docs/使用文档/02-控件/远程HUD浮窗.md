...
当前 `/qzuilib test` 进入 test 页面系统性重构期，旧“运行时远程 HUD”入口已清空。远程 HUD 交互 smoke 会按 `docs/开发者文档/规格文档/qzuilib测试页视觉矩阵规划.md` 的 RemoteNet 分组恢复：预期仍是打开远程 HUD 表单和弹幕，点击 HUD 内提交按钮后由服务端验证字段收集和 C2S 回调，
并显示通过或失败结果。
