...
- `UiHudLayerType.INTERACTIVE`：纯游戏内与容器类界面可见；暂停菜单、主菜单以及大多数原版 `Gui*` 页面会被归为 `MENU` 并直接隐藏。只有当前存在已打开的非菜单界面且鼠标已自由时，才会接通命中和焦点输入；纯游戏内锁鼠状态下仅可见，不可交互。
...
- 被动层会默认标记为整棵子树不可命中。
- 交互层当前不再限定必须是容器界面；但只有当前存在已打开的非菜单界面且鼠标未锁定时，才会接通输入。暂停菜单、主菜单以及大多数原版 `net.minecraft.client.gui.Gui*` 页面会落入 `MENU` 分类，因此既不会显示交互 HUD，也不会接通输入。纯游戏内锁定鼠标状态下，交互 HUD 仍只会显示，不会主动接管命中或键盘输入；当前主要适用容器类与部分自定义屏幕。
