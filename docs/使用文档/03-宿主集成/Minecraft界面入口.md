...
- 被动层会默认标记为整棵子树不可命中。
- 交互层在实现上仍会随纯游戏界面渲染，但只有容器态才会接通输入。当前 HUD 隐藏范围采用显式黑名单：游戏主页（含原版 `GuiMainMenu`、新版 `TitleScreen`、第三方主页 `galaxyspace.core.gui.GSGuiMainMenu`）、选图页、服务器列表、游戏内菜单和 Forge 配置页会落入 `MENU` 分类，因此既不会显示，也不会接通输入；
  未命中黑名单的第三方与大多数原版 `GuiScreen` 默认按容器态处理。
