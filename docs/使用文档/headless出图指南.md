...
- **每个产物从冷进程起算**：字体 atlas 是**进程级按需资源** —— 字形落在 atlas 的哪个位置取决于「此前
  ...
  但带上述历史依赖，只建议扫观感用）；
- **低概率输出抖动（已知，根因未定位）**：同一命令偶发产出两种结果之一。独立审核连跑 48 次：
  **47 次** `SHA256 a8fb1156c3978823…`（`commands=101 segments=62`）、**1 次**
  `2669a01c4c59cc42…`（`commands=100 segments=61`）⇒ 约 **2%**。（早先一次 6 次的小样本里出现过
  5:1，**该比率不可复现**：若真为 1/6，48 次里 ≤1 次的概率约 0.0017 —— 不要据小样本推断概率。）
  像素差异是滚动条滑块底端圆角的 8×6 px（30 像素，行剖面 2/2/4/8/8/6），与页面内容无关。
  故 **「逐字节相同」不是硬保证**：锚点对拍请连跑取多数值（≥5 次），或比对内容量
  （`bounds` / `commands` 数 / 文本字符数），不要用单次 hash 下结论。
