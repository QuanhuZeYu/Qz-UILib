...
- **每个产物从冷进程起算**：字体 atlas 是**进程级按需资源** —— 字形落在 atlas 的哪个位置取决于「此前
  ...
  但带上述历史依赖，只建议扫观感用）；
- **输出抖动（已定位并修复）**：同一命令曾偶发产出两种结果之一。定位（2026-09-19）：历史命令
  `--page=playground --page-index=8 --size=1280x720 --font-scale=100` 连跑 50 次得 **48 次**
  `a8fb1156c3978823…`（`commands=101 segments=62`）、**2 次** `2669a01c4c59cc42…`
  （`commands=100 segments=61`）⇒ 约 4%，两个 hash 与独立审核当时记录的一对**完全相同**；
  像素差异是滚动条滑块底端圆角的 8×6 px。
  ...
  **修复**：headless 会话解除该预算（`HeadlessSession.open` 里置
  `FontConfig.widthCacheMissBudgetPerWindow = 0`），全部码点走精确测量；同命令连跑 50 次 =
  **50:1**（全为 `a8fb1156…`，无第二种结果）。
  > 生产侧口径未变：游戏内该预算仍是「按时间窗降级」（卡顿时文本宽度会被近似）。这是**框架正确性**
  > 问题（同一输入的产出取决于机器负载），是否把降级口径改成与时间无关的量（按帧 / 按渲染代际）
  > 需单独决策。
