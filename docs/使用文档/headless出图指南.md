...
- **查询路径只推进 `--frames` 帧，默认 2 帧不够跑完输入脚本**：`--nodes` / `--find` / `--center` 不做出图那套
  稳定判据，只按 `--frames` 推进（出图默认 `--settle`/`--max-frames` 才有收敛循环）。所以「先点开对话框再看 `--nodes`」
  必须显式给足帧数（实测 `--frames=40` 足够点开对话框），否则脚本跑不完、查询到的是**点击之前**的树 ——
  而输出看起来完全正常，是个静默空测。写验收脚本时把 `--frames` 当必填项。
...
**坐标 vs 可点性是两个事实，各报各的**：`@x,y wxh` 一律是**未裁剪的绝对盒**（布局事实，便于排查「它为什么不在
画面上」）；「点得中吗」由 `[target]`/`[blocked]` 标注回答（命中事实）。二者会不一致，而那正是有用信息：
`@234,709 49x32 [blocked] hit=NONE` 读作「它在布局里占这块位置，但滚出视口了」。
