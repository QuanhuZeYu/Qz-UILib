...
**地址语义**：`r<树根序号>[/<子下标>]…`。树根序号是**宿主装配树根**的登记顺序（`getRoot()` 返回的那棵树，
不含 mount / portal 挂上去的内容根），子下标是父节点的子节点序号（同级按 z-order）。

地址在**同一环境档内稳定**：跨进程、跨分辨率、跨外观档、跨滚动、跨开关浮层都不变；**改字号倍率时坐标变而路径不变**。
（为什么不直接承诺「永远不变」：更早的一版拿「环境根登记顺序」当地址，而页重建会把内容根摘除后重新追加，
于是改字号 / 切页 / 点一次导航都会让 `r1` 与 `r2` 互换 —— 现已改为只按无父的装配树根编号。）
...
| 标注 | 含义 | 该不该点 |
|---|---|---|
| `[target]` | 可命中、有可见尺寸、且没有同为可命中的后代 | **是**，这是最深可点目标（按钮 / 列表项） |
| `[aggregate-name]` | 可命中容器（遮罩 / 卡片 / 树根），名称取自子树 | 否：点它多半落在遮罩或卡片上 |
| `[non-interactive]` | 不可命中（布局容器、装饰叶），仅 `--nodes=all` 会列出 | 否 |
...
**已知边界**：坐标是**未裁剪的绝对盒** —— 被滚动容器裁掉、或属于未激活浮层的节点仍按几何报坐标，
本功能不区分「在画面内 / 在树里」（`SceneGeometry.visibleBoxWithinScrollableAncestors` 提供该能力，
将来要做成另一个功能，不偷偷改 `--nodes` 的语义）。零尺寸节点在交互筛选下被剔除。
