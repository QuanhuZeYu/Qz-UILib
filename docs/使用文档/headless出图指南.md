...
- **每个产物从冷进程起算**：字体 atlas 是**进程级按需资源** —— 字形落在 atlas 的哪个位置取决于「此前
  ...
  但带上述历史依赖，只建议扫观感用）；
- **低概率输出抖动（已知，根因未定位）**：同一命令偶发产出两种结果之一。实测
  `--page=playground --page-index=8 --size=1280x720 --font-scale=100` 连跑 6 次：5 次
  `SHA256 a8fb1156c3978823…`（`commands=101 segments=62`）、1 次 `2669a01c4c59cc42…`
  （`commands=100 segments=61`）；实现者与独立审核两个会话都观测到**同一对**取值，
  像素差异是滚动条滑块底端圆角的 8×6 px，与页面内容无关。故 **「逐字节相同」不是硬保证**：
  锚点对拍请连跑数次取多数值，或比对内容量（`bounds` / `commands` 数 / 文本字符数），
  不要用单次 hash 下结论。
