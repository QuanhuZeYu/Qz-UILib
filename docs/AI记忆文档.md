...
- 背包页覆盖：hotbar/backpack 网格、自定义格子绘制、当前背包占用指标与返回原版背包按钮交互；该页按生产级前测试准备收口，不再承载临时迁移验证卡或动画/定位/inline 探针。`DocumentInventorySlotGridControl` 默认槽位底板使用不透明填充色，避免背后游戏画面直接穿透。
...
- `inventory_overview` 背包页已转为生产级前测试准备：保留快捷栏、主背包、占用指标、自定义格子绘制和返回按钮，移除临时迁移验证卡；后续优先在真实背包 UI 中收口槽位视觉、焦点、滚动、命中与真实数据渲染问题，其次再评估 dirty subtree / 细粒度缓存或 effect chain 后续优化。
- 不一次性开放全量布局动画。
