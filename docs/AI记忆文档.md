...
- 网络层运行时自检入口为 `/qzuilib test`，支持逐项或全部执行；真实 Channel / Fetch / Stream / Store 往返、C2S 分片、Fetch 错误/超时/取消/限流、玩家 Store、Store delta 检查已在当前 GTNH / ModularUI2 服务端环境完成一次基础联机验收（18/18 通过）；远程页面自检为交互 smoke，会触发服务端 `RemoteDocumentPages.open(...)` 并需要点击页面内提交按钮完成表单回传验证；Forge 回退仍需单独切换 `netTransport=forge` 验证。
