...
- 下一步主线应推进 CSS transition / animation 语义 MVP：巩固 transition 声明、动画属性注册、插值值类型、timeline tick、delay/duration/timing、动画生命周期、结束清理、运行值不写回 inline style、style invalidation 分层，以及 Smoke 页可见动画探针。优先覆盖 `opacity`、background/border/text color、border-radius 和至少一个长度类属性；layout-affecting 动画可先建立分类与失效策略，不必一次性开放全量。inline formatting 后续只处理阻塞动画或真实页面迁移的最小收口项，更完整图文混排、真实 baseline 细节、effect chain 的 multi tile atlas 批量路径、圆角裁剪、更完整 blur/saturate filter pass、采样尺寸限制和 FBO 不可用降级均后置。
