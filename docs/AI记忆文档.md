...
- 事件系统已支持标准 DOM 三阶段传播模型（capture → target → bubble）：所有事件类提供 `stopPropagation()`、`stopImmediatePropagation()`、`preventDefault()`；`ElementNode` 支持 capture handler 注册（`setCaptureClickHandler`/`setCaptureKeyHandler`/`setCaptureMouseDownHandler`/`setCaptureMouseUpHandler`）；`HtmlLikeDocumentWidget` 的 click/key/mousedown/mouseup 分发已按三阶段执行，其中 mousedown/mouseup/active/hover 由 `DocumentMouseEventDispatcher` 承载。
...
- 样式系统已支持后代选择器与子代选择器（`A B` / `A > B`）、按属性标记 `!important`（`UiStyleDeclaration.setImportant(UiStyleProperty)`）、`inherit` / `initial` / `unset` 级联关键字，以及最小 `::before` / `::after` 伪元素闭环。多个 `UiStyleSheet` 同特异性时以后挂载样式表优先，表内仍以后声明规则优先；类型化 setter 会替换同属性旧 keyword，避免 clear 后旧 keyword 复活。当前仍不支持逗号分组、兄弟组合器、属性选择器和 `@media`；状态伪类只基于当前计算目标的 `activeStates`，不提供祖先状态伪类联动。伪元素当前仅支持 `content` 文本、样式匹配、布局、绘制与命中回落到来源元素，不支持嵌套伪元素、attr()/counter()、独立事件目标或选择器查询返回伪元素节点。
...
- `UiDocument` / `ElementNode` 已补齐最小 DOM 平台能力：`createDocumentFragment()`、`cloneNode()` / `cloneNode(true)`、`addEventListener(...)` / `removeEventListener(...)` 与 `dispatchEvent(DocumentCustomEvent)` 可直接用于作者侧批量 DOM 操作、自定义事件派发和节点复制；当前 `cloneNode` 复制结构、属性、class 和 inline style，但不复制运行时事件监听器、交互 handler、自定义渲染回调或宿主运行态。
...
- `cursor` 已接通宿主真实系统光标：当前稳定支持 `default`、`pointer`、`text`、`move`、`not-allowed`、`wait`、`crosshair`、`ew-resize`、`ns-resize`、`none`；`grab` / `grabbing` 降级为 `move`，`help` 降级为 `default`。命中元素离开、没有命中元素，或命中链未声明 `cursor` 时会恢复默认箭头；系统光标宿主调用失败会降级为 no-op，不应打断输入链路。`DocumentCursorOverlayControl` 仍只是页面级图片覆层，不等同于系统光标。
...
- 样式系统文本排版控制：`letter-spacing`（字间距，可继承）、`word-break`（NORMAL/BREAK_ALL/KEEP_ALL，可继承）、`overflow-wrap`（NORMAL/BREAK_WORD/ANYWHERE，可继承）、`text-transform`（NONE/UPPERCASE/LOWERCASE/CAPITALIZE，可继承）、`text-indent`（可继承）、`text-shadow`（单值）已进入级联；`DocumentLayoutEngine` 已在文本换行阶段消费 `word-break` / `overflow-wrap` / `white-space` / `text-transform` / `text-indent`，覆盖普通英文、长 token、URL 标点、CJK 与中英文混排。`white-space:nowrap` 与 `pre` 优先禁止软换行，`pre` / `pre-wrap` 保留连续空白与显式换行，`pre-line` 折叠连续空白但保留显式换行；`text-overflow:ellipsis` 仍仅在 nowrap 且宽度受限时生效；`overflow-wrap:anywhere` 与 `word-break:break-all` 会影响 auto 宽固有尺寸，`overflow-wrap:break-word` 仅在受限宽度溢出时断长 token，不降低固有 auto 宽。
- `background-image` 已向作者层开放为 `UiBackgroundImage` 单图值，当前承载 `HostImageSource` 并在绘制阶段拉伸填充元素 border box、参与圆角裁剪；不提供 CSS `url(...)` 字符串解析、渐变、多背景、repeat/position/size 等完整 CSS 背景模型。
...
- `ElementNode` 已向页面作者公开程序化交互入口：`focus()`、`blur()`、`scrollTo(scrollLeft, scrollTop)`、`scrollIntoView()`、`getScrollLeft()`、`getScrollTop()`、`getMaxScrollLeft()`、`getMaxScrollTop()`，并配套 `getNextSibling()` / `getPreviousSibling()`。
