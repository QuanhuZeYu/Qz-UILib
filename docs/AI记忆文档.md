...
- `DocumentInventorySlotGridControl` 使用真实 HTML-like `table/tbody/tr/td` 表达背包格子底板，slot 背景/边框走标准 `BACKGROUND`/`BORDER` 绘制命令；支持 slot hover/active/selected 高亮、slot 点击回调、tooltip 文本 overlay 和可选鼠标携带物品 overlay；hover 中刷新槽位状态会同步刷新 tooltip 文本；自定义渲染回调仅用于登记 Minecraft 物品图标与鼠标携带物品延迟回放，复用 `ui.inventory` 下的 `InventorySlotGridLayout` 与 `InventorySlotGridItemRenderer`。
...
- 背包页覆盖：hotbar/backpack table 网格、slot DOM 底板、Minecraft 物品图标延迟回放、slot tooltip、真实左/右键物品移动、点击空白丢弃鼠标携带物品、单次鼠标携带物品 overlay、当前热键栏选中槽位高亮、当前背包占用指标与返回原版背包按钮交互；该页按生产级前测试准备收口，不再承载临时迁移验证卡或动画/定位/inline 探针。slot 默认底板允许半透明，视觉应叠在背包卡片/UI 底图上，不得通过改成不透明来规避穿透问题。
...
- `inventory_overview` 背包页已转为生产级前测试准备：保留快捷栏、主背包、占用指标、table slot DOM 底板、物品图标延迟回放、slot tooltip、真实物品移动、单次鼠标携带物品 overlay、当前热键栏选中槽位高亮和返回按钮，移除临时迁移验证卡；后续优先在真实背包 UI 中收口槽位视觉、焦点、滚动、命中与真实数据渲染问题，其次再评估 dirty subtree / 细粒度缓存或 effect chain 后续优化。
- 不一次性开放全量布局动画。
