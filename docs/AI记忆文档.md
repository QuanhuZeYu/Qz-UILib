...
- 已完成首期可复用 UI 主层快照服务：`UiScreenHostSession` 跨帧复用 `UiMainLayerSnapshotService`，`UiRenderContext.drawBackdropFilter(...)` 在同一帧内按 read framebuffer 复用主 UI 层 snapshot；Glass Lab 的 `Backdrop path` 诊断会显示 `snapshot=captured/reused WxH fbo=N`，多个 glass 元素正常情况下首个捕获，后续复用。
