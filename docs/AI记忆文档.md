...
- 阶段 4：建立 paint command、clip、scroll、deferred replay 与效果合成的统一渲染模型；当前 background/border/text/clip/backdrop-filter/paint-context command、首期 `DocumentEffectChain`、四阶段 stacking context/effect boundary 绘制/命中/滚动命中排序、paint-only transition 动画覆盖、paint-only 样式刷新不重排、首期 group opacity FBO 合成、首期可复用 UI 主层 snapshot、border-radius 运行圆角、`UiRenderContext` 投影、`overflow: auto` 滚动偏移、滚动条绘制/交互、命中测试与最小 smoke screen 集成已完成，下一步可继续把 renderer 运行时 effect pass、局部/分块 backdrop 采样模糊服务或更完整 inline layout 接入该模型。
...
- 下一步不应继续优先添加静态、易实现的动画属性；应优先推进更底层且会影响未来上限的基础建设，例如继续扩展完整 effect chain、`position: fixed`、absolute left+right/stretch 约束、或继续把当前 layout/paint 分层失效推进为更细粒度 style/layout/paint cache。Effect chain 方向后续应把 renderer 运行时 effect pass、局部/分块 snapshot、圆角裁剪、downsample blur/saturate 合成、采样尺寸限制和 FBO 不可用降级继续接入 `DocumentEffectChain`；也可继续清点剩余旧作者入口、补齐更完整 inline layout、列表/下拉类基础控件与可访问性语义。
