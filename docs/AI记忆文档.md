...
- 下一步不应继续优先添加静态、易实现的动画属性；当前优先推进 `position: fixed` 与 viewport containing block：基于现有 absolute containing block 和根视口滚动模型，让 fixed 元素相对 `HtmlLikeDocumentWidget` viewport 固定，不随文档滚动内容移动，并补齐绘制、命中测试与滚动命中一致性。之后再补 absolute left+right/stretch、inline formatting、effect chain 的 multi tile atlas 批量路径、圆角裁剪、更完整 blur/saturate filter pass、采样尺寸限制和 FBO 不可用降级；也可继续清点剩余旧作者入口、列表/下拉类基础控件与可访问性语义。
