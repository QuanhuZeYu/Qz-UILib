...
- 下一步不应继续优先添加静态、易实现的动画属性；`position: fixed`、absolute/fixed stretch 约束、text/span inline formatting、inline fragment 几何、inline margin/border/padding 盒边、inline fragment 背景/边框与 inline 文本命中初版已完成，下一步优先补 inline fragment 跨行圆角细节、更完整 inline box 尺寸、baseline/vertical-align 细节和图文组合。之后再继续 effect chain 的 multi tile atlas 批量路径、圆角裁剪、更完整 blur/saturate filter pass、采样尺寸限制和 FBO 不可用降级；也可继续清点剩余旧作者入口、列表/下拉类基础控件与可访问性语义。
