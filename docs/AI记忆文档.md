...
- HTML-like 动画基础设施已在 `club.heiqi.uilib.ui.animation` 起步；`DocumentAnimationClock` 提供可注入时间源，`DocumentAnimationTimeline` 负责在 computed style 基准值变化时创建 transition 覆盖层，并会按元素 computed `animation-name` 查找 `UiDocument` 注册的命名 `DocumentKeyframes` 创建 keyframe animation 覆盖层；动画运行值不会写回作者侧 inline style；运行中的 transition 如果作者侧清除 `transition-property` 或把 duration 改为 0，会在下一次 timeline 刷新时立即取消并回到 computed style 基准值，避免旧 transition 继续越权覆盖；作者侧 keyframe animation 当前支持 paint/effect 属性的多段 stop 轨道，例如 0%/50%/100%，并支持 delay、有限 iteration、fill-mode none/backwards/forwards/both 与 timing function；声明式 keyframe 数值 stop 会在启动时归一化到当前布局/效果约束，例如 opacity clamp 到 0..1、border-radius clamp 到当前布局盒半径上限、backdrop blur clamp 到 effect chain 最大值，避免与绘制阶段使用不同数值空间；当前 keyframe 声明重启条件固定为 `animation-name`、keyframes 定义对象、duration、delay、iteration count、fill-mode 或 timing function 变化，布局盒尺寸变化只刷新数值轨道 used value 归一化边界并保留原动画进度；同名 keyframes 定义被新对象替换时，引用该名称的元素会重启动画并使用新轨道，定义被移除时会取消动画并清掉旧 forwards fill；forwards fill 结束后若作者侧修改同属性 computed target，即使没有 transition 接管，也会按属性清除旧 fill 并让作者值接管，多属性 fill 只清理被作者改动的属性；当前固定运行值优先级为 `transition > keyframe animation > computed style`；`DocumentAnimationTimeline.hasRunningTransition(...)` 可按元素/属性查询当前是否有运行态 transition 覆盖，供 paint/effect 路径处理退场边界；`DocumentAnimationProperty` 当前已暴露 paint/effect/layout 影响范围分类，属性注册范围已扩展为 `BACKGROUND_COLOR`、`BORDER_COLOR`、`TEXT_COLOR`、`OPACITY`、`BORDER_RADIUS`、`BACKDROP_BLUR_RADIUS`、`WIDTH` 与 `HEIGHT`，其中 `BACKDROP_BLUR_RADIUS` 是首批 effect-affecting 长度类 transition，`WIDTH/HEIGHT` 是首批 layout-affecting transition 属性；当前 layout transition 首期只支持 px-to-px 数值路径，auto/% 不创建运行中 width/height transition。timing function 支持 linear、ease、ease-in、ease-out、ease-in-out 的简化插值。
...
- 已修复字体渲染底层 GL 资源泄漏：`FontRenderTool` 新增 `dispose()` 释放 1 个 VAO 与 6 个 VBO，`FontBatchRenderer` 暴露对应释放入口，`FontService.reload()` 现在会在字体系统重载时先释放旧批渲染底层资源再重新初始化，避免多次 reload 累积泄漏。
- 已抽取 `club.heiqi.uilib.gl.shader.ShaderProgramSupport` 统一承载 shader 文本读取、编译与 program 链接/验证逻辑，`FontShaderProgram` 与 `UiBackdropShaderProgram` 不再各自维护重复实现，后续 shader 失败信息与资源读取路径保持一致。
- 已将两条着色器兼容性争议项直接收敛为更保守实现：`fontF.frag` 去掉 uniform 初始化，统一依赖 Java 端设置运行值；`uiBackdropF.frag` 去掉 `lodBias` uniform 与三参数 `texture2D` 采样，回退为两参数采样，避免围绕 GLSL 版本细节产生误判或驱动分歧；本轮已验证 `./gradlew.bat --no-configuration-cache compileJava` 与 `./gradlew.bat --no-configuration-cache test --tests club.heiqi.uilib.ui.render.UiBackdropShaderProgramTest` 通过。
...
- 已完成首批 layout-affecting 动画底层准备和可见验收：`DocumentAnimationProperty` 新增 `WIDTH/HEIGHT`，`DocumentAnimationTimeline` 现在会跟踪 width/height 基准值并在 px-to-px 场景创建 layout transition；`DocumentLayoutEngine` 新增可选 `LayoutRuntimeValueResolver`，布局时会在解析 content width/height 后注入运行态布局值；`HtmlLikeDocumentWidget` 在检测到 layout 动画工作时会同帧重建运行态布局盒并刷新滚动范围，使 sibling 几何与 overflow 可随动画变化，layout transition 结束后恢复静态缓存；Smoke 页新增 `Layout animation probe: click blue card; width/height push sibling.` 游戏内探针，点击蓝色 `Layout card` 会在 92x34 与 190x58 之间做 `WIDTH/HEIGHT` transition，并推动右侧绿色 sibling 移动；纯 JVM 测试已覆盖属性分类、px-to-px 插值、auto->px 不建 transition、layout 动画期间 sibling 被推开、结束后恢复静态 paint cache，以及 Smoke 页 layout probe 文案、transition 声明和点击后的几何变化。
...
- 下一步主线应继续推进 CSS transition / animation 语义 MVP：在已有 paint/effect/layout 影响分类、`BACKDROP_BLUR_RADIUS` effect transition、`WIDTH/HEIGHT` layout transition 底层准备与 Smoke 可见探针、运行中取消语义、按属性运行状态查询、静态 paint cache 恢复、作者侧多段 keyframe stop MVP、keyframe 声明变更重启边界、布局尺寸变化不重启边界、keyframes 定义替换/移除边界和 forwards fill 作者目标变化接管边界基础上，下一步可谨慎评估更多 layout-affecting 属性或 keyframe layout 属性，但仍应限制在少量可控属性与明确 fallback，不一次性开放全量布局动画。inline formatting 后续只处理阻塞动画或真实页面迁移的最小收口项，更完整图文混排、真实 baseline 细节、effect chain 的 multi tile atlas 批量路径、圆角裁剪、更完整 blur/saturate filter pass、采样尺寸限制和 FBO 不可用降级均后置。
