...
- 下一步主线应继续推进 CSS transition / animation 语义 MVP：在已有 paint/effect/layout 影响分类和 `BACKDROP_BLUR_RADIUS` effect transition 基础上，继续巩固 transition 声明、动画属性注册、插值值类型、timeline tick、delay/duration/timing、动画生命周期、结束清理、运行值不写回 inline style、style invalidation 分层，以及 Smoke 页可见动画探针。layout-affecting 动画可先建立分类与失效策略，不必一次性开放全量。inline formatting 后续只处理阻塞动画或真实页面迁移的最小收口项，更完整图文混排、真实 baseline 细节、effect chain 的 multi tile atlas 批量路径、圆角裁剪、更完整 blur/saturate filter pass、采样尺寸限制和 FBO 不可用降级均后置。
- 动画 MVP 验收必须包含纯 JVM 测试与游戏内 Smoke 可见探针；测试重点覆盖 transition 创建、插值、delay/duration/timing、完成清理、paint-only 不重排、动画结束后恢复静态缓存路径，以及动画期间避免每帧无条件全量 layout/paint 重建。
