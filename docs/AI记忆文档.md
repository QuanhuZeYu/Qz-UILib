...
- 后续宿主真实效果策略：在现有同帧局部 UI 主层快照和 downsample filter pass 基础上继续推进分块 snapshot 与更完整 separable blur/box blur 近似；继续限制最大 blur radius 和最大采样尺寸；FBO 或 shader 不可用时继续回退为 raw snapshot、半透明 tint、高光边和投影的伪玻璃视觉。不要把元素级 backdrop-filter 设计成直接模糊游戏世界画面。
...
- 已完成首期可复用 UI 主层快照服务并修正旧快照过度复用问题：`UiScreenHostSession` 跨帧复用 `UiMainLayerSnapshotService`，`UiRenderContext.drawBackdropFilter(...)` 在同一帧内按 read framebuffer、局部采样区域、UI 主层内容版本与 blur 级别复用 snapshot；大半径 blur 会在局部 snapshot 后生成降采样 filter 纹理，shader 半径和 mip lod 会按降采样倍率折算，FBO 不可用时回退到 raw snapshot；Glass Lab 的 `Backdrop path` 诊断会显示 `snapshot=captured/reused WxH @x,y fbo=N rev=R filter=raw/downsampleN ...`，多个 glass 元素只有在两次采样之间没有新的 UI 绘制写入且采样区域相同时才复用，否则必须重新捕获。
...
- 阶段 4：建立 paint command、clip、scroll、deferred replay 与效果合成的统一渲染模型；当前 background/border/text/clip/backdrop-filter/paint-context command、首期 `DocumentEffectChain`、renderer 运行时 effect pass 类型化回放、四阶段 stacking context/effect boundary 绘制/命中/滚动命中排序、paint-only transition 动画覆盖、paint-only 样式刷新不重排、首期 group opacity FBO 合成、首期可复用局部 UI 主层 snapshot、首期 downsample filter pass、border-radius 运行圆角、`UiRenderContext` 投影、`overflow: auto` 滚动偏移、滚动条绘制/交互、命中测试与最小 smoke screen 集成已完成，下一步可继续把分块 backdrop 采样模糊服务、更完整 blur/filter pass 或更完整 inline layout 接入该模型。
