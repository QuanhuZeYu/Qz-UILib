...
- 旧代码清理建议优先从旧页面宿主链路开始：`UiTestScreen`、`InventoryOverviewScreen`、`ControllerBackedDocumentScreen`、`BaseDocumentScreen`，再移除 `DocumentPageAuthoringSurface.adapt(DocumentPageWidget)` 与 `DocumentPageWidgetAuthoringAdapter`，之后收缩 `DocumentUiScope` 旧 retained widget factory。
