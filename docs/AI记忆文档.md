...
- 样式系统已支持后代选择器与子代选择器（`A B` / `A > B`）、按属性标记 `!important`（`UiStyleDeclaration.setImportant(UiStyleProperty)`）以及 `inherit` / `initial` / `unset` 级联关键字（`UiStyleDeclaration.setKeyword(...)`）。多个 `UiStyleSheet` 同特异性时以后挂载样式表优先，表内仍以后声明规则优先；类型化 setter 会替换同属性旧 keyword，避免 clear 后旧 keyword 复活。当前仍不支持逗号分组、兄弟组合器、属性选择器、伪元素和 `@media`；状态伪类只基于当前计算目标的 `activeStates`，不提供祖先状态伪类联动。
