...
- scene 新栈有一批「违反即阻断合并」硬约束（控件契约 R1-R12、宪章不变量 I1-I12、
  ...
  写/评审 scene 代码前必须先读硬约束总目录 `docs/架构/硬约束总目录.md` 并按其指针核对，不得在目录外私自绕过任何一条。
...
- 按变更内容写回对应层，优先覆盖/归并而非按日期追加：
  ...
  - 进展 `docs/进展/`：现状、计划、交接，高频覆盖
  - 架构 `docs/架构/`：目录结构、边界、硬约束、技术债、测试体系、稳定命令、项目约定
  - 决策 `docs/决策/`：关键取舍、边界约束、重要设计
  - 错误预防 `docs/开发者文档/错误预防.md`：按类别沉淀通则
- 只沉淀对后续协作有持续价值的事实；临时调试、一次性试验结论不单独留档
- 信息能稳定归属到某层时写回原文档，不复制进总导航
- 完成独立任务后至少检查一次上述各层是否需更新；会话结束/交接必更新 `docs/进展/交接.md`
...
### 测试规范
- 新建/评审 scene 测试前必读 `docs/架构/测试体系约定.md`（分层总纲、L2 防错清单、断言库速查、输入两入口边界）；L2 纯数学边界靠评审纪律守卫，按其 §6 逐条核对

## 6. 错误记录规范
- 开发中发现的错误、反复问题、重要教训必须及时记录，避免重复踩坑
- 索引文件 `docs/开发者文档/错误预防.md` 是唯一入口：二级标题作指针，详情落 `ERROR-<日期>-<简述>.md`
...
- shell 编译命令与 GRADLE_USER_HOME 等细节见 `docs/架构/稳定命令.md`（PowerShell 不支持 `&&`，链式用 `;`）
