...
/**
 * vanilla 连接生命周期握手时序测试。
 */
public class VanillaConnectionLifecycleTest {
    ...
    public void setUp() throws Exception {
        resetNetService();
        VanillaConnectionLifecycle.getInstance().resetForTests();
    ...
    @Test
    public void shouldOnlyCacheClientNetworkManagerDuringPlayHandlerConstruction() throws Exception {
        NetworkManager networkManager = new NetworkManager(true);

        VanillaMixinTransport.onClientHandshakeReady(networkManager);

        Assert.assertEquals(0, outboundQueueOf(networkManager).size());
    }
    ...
    public void shouldSendClientCapabilityHandshakeAfterFmlConnectionEstablishedOnlyOnce() {
        ...
        VanillaConnectionLifecycle.getInstance().onClientConnectionEstablished(networkManager);
        VanillaConnectionLifecycle.getInstance().onClientConnectionEstablished(networkManager);
    ...
    public void shouldQueueServerCapabilityHandshakeAfterFmlConnectionEstablishedOnlyOnce() {
        ...
        VanillaConnectionLifecycle.getInstance().onServerConnectionEstablished(networkManager, player);
        VanillaConnectionLifecycle.getInstance().onServerConnectionEstablished(networkManager, player);
    ...
    public void shouldResolveServerPlayerFromPlayHandler() throws Exception {
        ...
        Assert.assertSame(player, VanillaConnectionLifecycle.resolveServerPlayer(handler));
    }
    ...
    public void shouldAllowNewHandshakeAfterDisconnectCleanup() {
        ...
        VanillaConnectionLifecycle.getInstance().onClientConnectionEstablished(firstManager);
        VanillaConnectionLifecycle.getInstance().onClientDisconnected();
        VanillaConnectionLifecycle.getInstance().onClientConnectionEstablished(secondManager);
