 ...
 *
 * <p>只锚定命令元信息、tab 补全、子命令解析与非法参数兜底;开屏路径(test/glass/modernconfig)与
...
public class QzUiLibClientCommandTest {
    ...
    /**
     * 验证命令元信息与补全结果(test + glass + modernconfig + chatmd)。
     */
    @Test
    public void shouldExposeCommandMetadataAndTabCompletion() {
        ...
        Assert.assertEquals("/qzuilib <test|glass|modernconfig|chatmd on|off|status>", command.getCommandUsage(sender));
        ...
        Assert.assertTrue(command.addTabCompletionOptions(sender, new String[] { "gl" }).contains("glass"));
    ...
    public void resolveSubcommandParsesKnownAndRejectsUnknown() {
        ...
        Assert.assertEquals(QzUiLibClientCommand.Subcommand.TEST,
                QzUiLibClientCommand.resolveSubcommand(new String[] { "TEST" }));
        Assert.assertEquals(QzUiLibClientCommand.Subcommand.GLASS,
    ...
    public void shouldRejectArgumentsOtherThanKnownSubcommands() {
        ...
        try {
            ...
            Assert.assertEquals("/qzuilib <test|glass|modernconfig|chatmd on|off|status>", expected.getMessage());
        }
