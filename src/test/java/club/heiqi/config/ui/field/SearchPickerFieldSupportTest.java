...
public class SearchPickerFieldSupportTest {
    ...
    /** 结果列表行容器：窗口化后 viewport = [content]，content = [topSpacer, rowsContainer, bottomSpacer]。 */
    private static SceneNode gridRows(SceneNode panel) {
        return gridViewport(panel).__getChildren().get(0).__getChildren().get(1);
    }

    /** 窗口内结果单元。 */
    private static SceneNode gridCell(SceneNode panel, int index) {
        SceneNode rowsContainer = gridRows(panel);
