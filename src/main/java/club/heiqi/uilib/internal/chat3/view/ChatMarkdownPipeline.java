...
/**
 * chat3 消息级 markdown 段流管道（M5 接线本体；规划《通用Markdown渲染器》§三 M5/§二 L3）。
 *
 * <p><b>链路（顺序即门禁 M4 B 路定义，一字不差）</b>：消息原文 →
 * {@link MarkdownDocument#parse(String)}（L1 块级 + 行内；列表「• 」、引用「&gt; 」剥除、
 * 块级公式、行内 code 衬底/12px、F2 缩进、F6 块间距占位段全部由 L1/L2 承接）→
 * {@link MarkdownDocument#toSegments(MarkdownStyleTable, TextStyle)}（样式表恒由
 * {@link #chatStyleTable()} 构建——引用色取 {@link ChatMarkdownSettings#getTextSecondaryArgb()}
 * 现值，G4 度量/配色单一登记面）→ <b>§ 桥</b>（{@link #bridgeSectionCodes}：保住 chat3 现有
 * § 颜色语义；「markdown 不引入颜色」旧裁定不变——颜色恒由 § 码 + 基础样式决定，markdown 只叠加
 * 样式位）→ {@link ChatUrlLinkifier#linkify}（<b>换行前</b>整条流链接化：跨显示行长 URL 的
 * link 值天然完整，旧 UrlChain 续链机制对气泡路不再需要）→
 * {@link MarkdownPainter#wrapLines}（L2 换行，度量 = 注入 TextLayoutService，零自设常量）。</p>
 *
 * <p><b>每帧零解析（规划 §六 3）</b>：两级 LRU 缓存沿用 {@code ChatLineLayouter} 既有布局缓存
 * 纪律——扁平段流 key = 原文@基础色#配色代（解析/桥/链接化与字体无关，配色变更即时失效）；
 * 视觉行 key = 原文@基础色#配色代#定行宽#字号#度量纪元（{@code FontService.getRuntimeVersion()}，
 * 字体 reload 整体失效，与度量纪元同口径）。渲染帧只在结构重建时命中缓存，不逐帧 parse。</p>
 *
 * <p>缓存按实例隔离（与 {@link ChatMessageList} 的段解析缓存同纪律，测试注入互不串味）。
 * 系统消息不走本管道（§3.5 排版规则仅作用于气泡内，行级旧行为原样保留在
 * {@link ChatMessageList} 的系统路）。</p>
 */
final class ChatMarkdownPipeline {

    /** 扁平段流缓存上限（历史 100 行 + 配色切换余量）。 */
    private static final int FLAT_CACHE_MAX = 200;
    /** 视觉行缓存上限（同 {@code ChatLineLayouter.MAX_ENTRIES} 口径）。 */
    private static final int LINES_CACHE_MAX = 160;

    /** 扁平段流缓存：原文@基础色#配色代 → 链接化后的扁平段流（含 \n 分隔段与 F6 占位段）。 */
    private final Map<String, List<TextSegment>> flatCache = newLru(FLAT_CACHE_MAX);
    /** 视觉行缓存：扁平 key#定行宽#字号#度量纪元 → wrapLines 产物。 */
    private final Map<String, List<List<TextSegment>>> linesCache = newLru(LINES_CACHE_MAX);

    /**
     * 消息原文 → 显示行段流（每行一个 {@code List<TextSegment>}，行内无 {@code \n}）。
     *
     * @param messageText   去前缀消息原文（{@code ChatCardComposer.MessageLines.getDisplayText()}）
     * @param baseColor     气泡正文基础色（ARGB）
     * @param maxWidthPx    定行宽（与行切分器同口径；{@code <= 0} = 只按 {@code \n} 硬断）
     ...
     *                      null = 生产路 {@link MarkdownPainter#wrapLines} +
     *                      {@code FontService} 度量——与 {@code uiLibMeasure}/{@code uiLibSegmentMeasurer}
     *                      三者同源，钳宽/换行/渲染一把尺）
     * @return 不可变视觉行列表（至少一行；空文本 → 单空行）
     */
    synchronized List<List<TextSegment>> layout(String messageText, int baseColor, int maxWidthPx,
        ...
        List<TextSegment> flat = flatCached(text, baseColor);
        int epoch = FontService.getInstance().getRuntimeVersion();
        String key = cacheKey(text, baseColor) + '#' + maxWidthPx + '#' + fontSizePx + '#' + epoch
                + (wrapOverride == null ? "" : "#w");
        List<List<TextSegment>> hit = linesCache.get(key);
        if (hit != null) {
            return hit;
        }
        List<TextSegment> processed = postProcessor == null
                ? flat : postProcessor.postProcess(flat, ChatMarkdownSettings.getChatFontSizePx());
        List<List<TextSegment>> lines = wrapOverride != null
                ? wrapOverride.wrap(processed, maxWidthPx, fontSizePx)
                : MarkdownPainter.wrapLines(processed,
                        FontService.getInstance().getTextLayoutService(), maxWidthPx, fontSizePx);
        linesCache.put(key, lines);
        return lines;
    }

    /** 解析 → 桥 → 链接化（换行前），带扁平缓存。 */
    private List<TextSegment> flatCached(String text, int baseColor) {
        String key = cacheKey(text, baseColor);
        List<TextSegment> hit = flatCache.get(key);
        if (hit != null) {
            return hit;
        }
        TextStyle base = new TextStyle();
        base.setColor(baseColor);
        List<TextSegment> segments = MarkdownDocument.parse(text).toSegments(chatStyleTable(), base);
        segments = bridgeSectionCodes(segments);
        segments = ChatUrlLinkifier.linkify(segments, ChatMarkdownSettings.getLinkArgb());
        flatCache.put(key, segments);
        return segments;
    }
    ...
    /**
     * § 桥：把 toSegments 产物里残留在段文本中的 § 样式码解释为样式（chat3 现有 § 颜色语义
     ...
     *
     * @param segments toSegments 产物（只读）
    ...
    /** 视觉行是否引用行（M5 结构判据：首个可视段颜色 = 引用降色现值——该色由 L1 F3 旋钮写入，
     *  普通段/标题/列表段恒为基础色，二者不同色即不同判）。 */
    ...
    /** 视觉行是否块级公式独占行（M5 结构判据：整行恰为一个 latex 原子段——L1 行内 $/$$ 解析
     *  产物，与旧 ChatMarkdownLineRule「$$ 开头 / $…$ 独占行」同集合；行内混排公式恒 false）。 */
    static boolean isBlockMathRow(List<TextSegment> line) {
        return line.size() == 1 && line.get(0).isLatex();
    }

    /**
     * HUD 形态 8 行截断（设计稿 §5.4，M5 起作用于 L2 视觉行）：超过 8 行保留前 8 行，
     * 末行段流尾部追加省略号（与 {@code ChatCardComposer.ELLIPSIS} 同款；行宽可用时有度量
     * 注入则先逐码点回退再补，保持「省略号不撑爆行」旧口径）。
     */
    static List<List<TextSegment>> clampHudLines(List<List<TextSegment>> lines,
            ChatMessageList.SegmentMeasurer measurer, int fontSizePx, int maxWidthPx) {
        int max = ChatCardComposer.HUD_MAX_LINES;
        if (lines.size() <= max) {
            return lines;
        }
        List<List<TextSegment>> out = new ArrayList<List<TextSegment>>(max);
        for (int i = 0; i < max - 1; i++) {
            out.add(lines.get(i));
        }
        List<TextSegment> last = new ArrayList<TextSegment>(lines.get(max - 1));
        appendEllipsis(last, measurer, fontSizePx, maxWidthPx);
        out.add(Collections.unmodifiableList(last));
