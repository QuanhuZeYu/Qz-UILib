...
public final class QzUiLibModernSchema {
    ...
    public static ConfigSchema create() {
        return ConfigSchema.builder("qzuilib")
                ...
                .section("fontSystem")
                    .title("Font System")
                    .number("lerpMode").defaultValue(Double.valueOf(3.0)).range(0, 3).slider()
                        .label("lerpMode").helper("插值模式").build()
                    .number("aaMode").defaultValue(Double.valueOf(2.0)).range(1, 2).slider()
                    ...
                    .number("smoothRangeMin").defaultValue(Double.valueOf(0.0)).range(0, 1).slider(0.1)
                        .label("smoothRangeMin").helper("平滑下界").build()
                    .number("smoothRangeMax").defaultValue(Double.valueOf(0.9)).range(0, 1).slider(0.1)
                    ...
                    .number("aaStrength").defaultValue(Double.valueOf(12.0)).range(1, 120).slider()
                ...
                .section("fontSizeSetting")
                    .title("Font Size")
                    .number("awtCharSize").defaultValue(Double.valueOf(64.0)).range(8, 256).slider()
                        .label("awtCharSize").helper("字符生成分辨率；与 charSize 的比值是字体层缩放因子").build()
                    .number("charSize").defaultValue(Double.valueOf(9.0)).range(1, 72).slider(0.5)
