...
public final class PickerMetrics {
     ...
     * 声明前再夹一次），副作用是<b>设定与产出不符</b>：按旧公式遍历声明 0..256 × 倍率 0..200，
     * 10537 对里有 7033 对（66.75%）渲染字号 ≠ 派生几何字号 —— 例如声明 8 × 100% ⇒ 几何按 11 算；
     * 声明 16 × 200% ⇒ 文字 32 / 几何 24（同一声明在域内相乘、在域外被截）。
     * 字号域的唯一边界是 {@link FontSizeLimits} 的 {@code [0, 256]}，本控件不再叠加第二套区间 ——
