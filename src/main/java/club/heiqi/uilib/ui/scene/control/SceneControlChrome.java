 ...
 * <p>交互控件层高频出现三段一字不差的 {@code rt.bind(Computed.create(...), setter)} 样板：
 ...
 * <h3>守 R4 / I4 / I1</h3>
 * <ul>
 *   <li><b>R4</b>：helper 仅薄包装 {@link SceneRuntime#bindComputed}（等价
 *       {@code bind(Computed.create(...), setter)}），外观仍「经 bind 派生」，不在 handler 里
 *       命令式 setXxx。</li>
...
public final class SceneControlChrome {
     ...
     *     node::setBorderColor);
     * }</pre>
     * 内部走 {@link SceneRuntime#bindComputed}，setter {@code setBorderColor} 自动打 PAINT 级（I4）。</p>
     *
     * @param rt          场景运行时（提供 bindComputed）
     ...
     * <p>等价样板：
     * <pre>{@code
     * rt.bind(Computed.create(() -> Boolean.TRUE.equals(selected.get())
     *         ? SceneStateColors.selectedBackground(en, hovered, pressed)
     *         : SceneStateColors.standardBackground(en, hovered, pressed)),
     *     node::setBackgroundColor);
     * }</pre>
     * 内部走 {@link SceneRuntime#bindComputed}，setter {@code setBackgroundColor} 自动打 PAINT 级（I4）。</p>
     *
