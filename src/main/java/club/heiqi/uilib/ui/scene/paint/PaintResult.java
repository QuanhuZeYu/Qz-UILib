 ...
 * 使引擎逐步走向无状态化（渲染出口只认抽象契约，并行强化方向）。</p>
 *
 * <h3>不可变契约</h3>
 * <ul>
 *   <li>{@link #plan} 是 paint 产出的自包含 Display List，可延迟 replay（不含任何节点/scene 类型）。</li>
 *   <li>{@link #regeneratedFragmentCount} 是本次 paint 调用中重新生成的 fragment 数量，
 *       仅供测试断言 fragment 缓存复用行为，生产代码不应依赖。</li>
 * </ul>
 *
...
public final class PaintResult {
    ...
    /** 本次 paint 重新生成的 fragment 数量（测试探针，fragment 缓存复用断言用）。 */
