 ...
 *
 * <p>档位只定义两件事：<b>图标目标边长</b>与<b>基准字号</b>。其余几何（cellW / trackH / stride /
 * gap / pad / labelGap）一律由 {@link PickerMetrics} 从「逻辑盒 + 字号 + 档位」派生，
 * 档位本身不携带任何布局像素（禁止把档位实现成"一套固定尺寸表"）。</p>
 *
 * <ul>
...
public enum PickerDensity {
    ...
    /** @return 基准字号（逻辑 px；再乘用户字号倍率并夹取到字号域） */
