...
/**
 * Forge/FML 回退传输的连接生命周期桥。
 */
public final class ForgeConnectionLifecycle {

    private static final ForgeConnectionLifecycle INSTANCE = new ForgeConnectionLifecycle();
    ...
    private ForgeConnectionLifecycle() {}
     ...
     * @return 单例
     */
    public static ForgeConnectionLifecycle getInstance() {
        return INSTANCE;
    }
    ...
    public void onServerConnected(FMLNetworkEvent.ServerConnectionFromClientEvent event) {
        ...
        if (player == null) {
            MyMod.LOG.warn("Qz Forge transport 无法解析服务端连接玩家，跳过能力握手：handler={}",
    ...
    void onClientConnectionEstablished(NetworkManager networkManager) {
        ...
        try {
            MyMod.LOG.debug("Qz Forge transport client FML connection established: {}", networkManager);
            NetService.getInstance().sendCapabilityHandshakeToServer();
        } catch (RuntimeException exception) {
            MyMod.LOG.warn("Qz Forge 客户端能力握手发送失败", exception);
        }
    }
    ...
    void onServerConnectionEstablished(final NetworkManager networkManager, final Object player) {
        ...
        NetService.getInstance().runOnMainThread(NetSide.SERVER, new Runnable() {
            ...
            public void run() {
                try {
                    MyMod.LOG.debug("Qz Forge transport server FML connection established: {}", player);
                    NetService.getInstance().sendCapabilityHandshakeToPlayer(player);
                } catch (RuntimeException exception) {
                    MyMod.LOG.warn("Qz Forge 服务端能力握手发送失败：player={}", String.valueOf(player), exception);
                }
            }
        });
    }
