...
public class SVGCanvas {
    ...
    /**
     * Returns a new canvas that will generate SVG commands from its draw calls, and
     * send
     * them to the provided stream. Ownership of the stream is not transfered, and
     * it must
     ...
     *
     * @param bounds defines an initial SVG viewport (viewBox attribute on the root
     *               SVG element).
     ...
     * or complete until the canvas instance is deleted.
     *
     * @param bounds             defines an initial SVG viewport (viewBox attribute
    ...
    public static Canvas make(@NotNull Rect bounds, @NotNull WStream out, boolean convertTextToPaths,
            boolean prettyXML) {
        Stats.onNativeCall();
        long ptr = _nMake(bounds._left, bounds._top, bounds._right, bounds._bottom, Native.getPtr(out),
                0 | (convertTextToPaths ? 1 : 0) | (prettyXML ? 0 : 2));
