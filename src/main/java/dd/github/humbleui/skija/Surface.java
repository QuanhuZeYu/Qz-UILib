...
public class Surface extends RefCnt {
    ...
    /**
     * <p>
     * Allocates raster Surface. Canvas returned by Surface draws directly into
     * pixels.
     * </p>
     *
     * <p>
     ...
     *
     * <ul>
     * <li>info dimensions are greater than zero;</li>
     * <li>info contains ColorType and AlphaType supported by raster surface;</li>
     * <li>pixelsPtr is not 0;</li>
     * <li>rowBytes is large enough to contain info width pixels of ColorType.</li>
     * </ul>
     *
     * <p>
     ...
     *                  of raster surface; width and height must be greater than
     *                  zero
    ...
    public static Surface wrapPixels(@NotNull ImageInfo imageInfo,
        ...
        try {
            ...
            long ptr = _nWrapPixels(
                    ...
                    surfaceProps);
            if (ptr == 0) {
                throw new IllegalArgumentException(String.format("Failed Surface.wrapPixels(%s, %d, %d, %s)", imageInfo,
                        pixelsPtr, rowBytes, surfaceProps));
            }
        ...
        try {
            ...
            if (ptr == 0) {
                throw new IllegalArgumentException(
                        String.format("Failed Surface.wrapPixels(%s, %s)", pixmap, surfaceProps));
            }
            return new Surface(ptr);
        } finally {
     ...
     * Allocates raster Surface. Canvas returned by Surface draws directly into
     * pixels.
     * Allocates and zeroes pixel memory. Pixel memory size is imageInfo.height()
     * times imageInfo.minRowBytes().
     ...
     * <li>info dimensions are greater than zero;</li>
     * <li>info contains ColorType and AlphaType supported by raster surface;</li>
     * <li>rowBytes is large enough to contain info width pixels of ColorType, or is
     * zero.</li>
     * </ul>
     *
     * <p>
    ...
    public static Surface makeRaster(@NotNull ImageInfo imageInfo,
        ...
        try {
            ...
            if (ptr == 0)
                throw new IllegalArgumentException(
                        String.format("Failed Surface.makeRaster(%s, %d, %s)", imageInfo, rowBytes, surfaceProps));
            return new Surface(ptr);
        } finally {
            ReferenceUtil.reachabilityFence(imageInfo._colorInfo._colorSpace);
        }
    ...
    /**
     * @deprecated - use
     *             {@link #wrapBackendRenderTarget(DirectContext, BackendRenderTarget, SurfaceOrigin, SurfaceColorFormat, ColorSpace)}
     */
    @Deprecated
     ...
     *
     * <p>
     * {@link Surface} is returned if all parameters are valid. backendRenderTarget
     * is valid if its pixel
     * configuration agrees with colorSpace and context;
     * for instance, if backendRenderTarget has an sRGB configuration, then context
     * must support sRGB,
     * and colorSpace must be present. Further, backendRenderTarget width and height
     * must not exceed
     * context capabilities, and the context must be able to support back-end render
     * targets.
     * </p>
     *
     * @param context     GPU context
     * @param rt          texture residing on GPU
     * @param origin      surfaceOrigin pins either the top-left or the bottom-left
     ...
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_MakeFromBackendTexture">https://fiddle.skia.org/c/@Surface_MakeFromBackendTexture</a>
     */
    @NotNull
    ...
    /**
     * @deprecated - use
     *             {@link #wrapBackendRenderTarget(DirectContext, BackendRenderTarget, SurfaceOrigin, SurfaceColorFormat, ColorSpace, SurfaceProps)}
     */
    @Deprecated
    public static Surface makeFromBackendRenderTarget(DirectContext context,
            BackendRenderTarget rt,
    ...
    public static Surface wrapBackendRenderTarget(@NotNull DirectContext context,
        ...
        try {
            ...
            Stats.onNativeCall();
            long ptr = _nWrapBackendRenderTarget(Native.getPtr(context), Native.getPtr(rt), origin.ordinal(),
                    colorFormat.ordinal(), Native.getPtr(colorSpace), surfaceProps);
            if (ptr == 0)
                throw new IllegalArgumentException(
                        String.format("Failed Surface.wrapBackendRenderTarget(%s, %s, %s, %s, %s)", context, rt, origin,
    ...
    /**
     * @deprecated - use
     *             {@link #wrapMTKView(DirectContext, long, SurfaceOrigin, int, SurfaceColorFormat, ColorSpace, SurfaceProps)}
     */
    @Deprecated
    ...
    public static Surface wrapMTKView(@NotNull DirectContext context,
        ...
        try {
            ...
            Stats.onNativeCall();
            long ptr = _nWrapMTKView(Native.getPtr(context), mtkViewPtr, origin.ordinal(), sampleCount,
                    colorFormat.ordinal(), Native.getPtr(colorSpace), surfaceProps);
            if (ptr == 0)
                throw new IllegalArgumentException(String.format("Failed Surface.WrapMTKView(%s, %s, %s, %s, %s, %s)",
                        context, mtkViewPtr, origin, colorFormat, colorSpace, surfaceProps));
     ...
     *
     *             <p>
     *             Canvas returned by Surface draws directly into pixels. Allocates
     *             and zeroes pixel memory.
     *             Pixel memory size is height times width times four. Pixel memory
     *             is deleted when Surface is deleted.
     *             </p>
     *
     *             <p>
     *             Internally, sets ImageInfo to width, height, native color type,
     *             and ColorAlphaType.PREMUL.
     *             </p>
     *
     *             <p>
     ...
     *
     *             <p>
     *             Use to create Surface that matches PMColor, the native pixel
     ...
     * @return Surface if all parameters are valid; otherwise, null
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_MakeRasterN32Premul">https://fiddle.skia.org/c/@Surface_MakeRasterN32Premul</a>
     */
    @NotNull
    @Deprecated
     ...
     *
     * @param context      GPU context
     * @param budgeted     selects whether allocation for pixels is tracked by
     *                     context
     * @param imageInfo    width, height, ColorType, AlphaType, ColorSpace;
     *                     width, or height, or both, may be zero
     * @param sampleCount  samples per pixel, or 0 to disable full scene
     *                     anti-aliasing
     * @param surfaceProps LCD striping orientation and setting for device
     *                     independent
     *                     fonts; may be null
    ...
    public static Surface makeRenderTarget(@NotNull DirectContext context,
            ...
            int sampleCount,
            @Nullable SurfaceProps surfaceProps) {
        return makeRenderTarget(context, budgeted, imageInfo, sampleCount, SurfaceOrigin.BOTTOM_LEFT, surfaceProps,
                false);
    }
     ...
     * @param sampleCount  samples per pixel, or 0 to disable full scene
     *                     anti-aliasing
     * @param origin       pins either the top-left or the bottom-left corner to the
     *                     origin.
     * @param surfaceProps LCD striping orientation and setting for device
     *                     independent
     *                     fonts; may be null
     ...
     *
     * <p>
     * shouldCreateWithMips hints that Image returned by
     * {@link #makeImageSnapshot()} is mip map.
     * </p>
     *
     * @param context              GPU context
     * @param budgeted             selects whether allocation for pixels is tracked
     *                             by context
     * @param imageInfo            width, height, ColorType, AlphaType, ColorSpace;
     *                             width, or height, or both, may be zero
     * @param sampleCount          samples per pixel, or 0 to disable full scene
     *                             anti-aliasing
     * @param origin               pins either the top-left or the bottom-left
     *                             corner to the origin.
     * @param surfaceProps         LCD striping orientation and setting for device
     *                             independent
     *                             fonts; may be null
    ...
    public static Surface makeRenderTarget(@NotNull DirectContext context,
        ...
        try {
            ...
            if (ptr == 0)
                throw new IllegalArgumentException(
                        String.format("Failed Surface.makeRenderTarget(%s, %b, %s, %d, %s, %s, %b)", context, budgeted,
    ...
    /**
     * Returns Surface without backing pixels. Drawing to Canvas returned from
     * Surface
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_MakeNull">https://fiddle.skia.org/c/@Surface_MakeNull</a>
     */
    @NotNull
    @Contract("_, _ -> new")
     ...
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_width">https://fiddle.skia.org/c/@Surface_width</a>
     */
    public int getWidth() {
        try {
            Stats.onNativeCall();
     ...
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_height">https://fiddle.skia.org/c/@Surface_height</a>
     */
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_notifyContentWillChange">https://fiddle.skia.org/c/@Surface_notifyContentWillChange</a>
     */
     ...
     *
     * <p>
     * Returned Surface contains the same raster, GPU, or null properties as the
     * original.
     * Returned Surface does not share the same pixels.
     * </p>
     *
     * <p>
     * Returns null if imageInfo width or height are zero, or if imageInfo is
     ...
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_makeSurface">https://fiddle.skia.org/c/@Surface_makeSurface</a>
     */
    @Nullable
     ...
     * Image allocation is accounted for if Surface was created with
     * SkBudgeted::kYes.
     * </p>
     *
     * @return Image initialized with Surface contents
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_makeImageSnapshot">https://fiddle.skia.org/c/@Surface_makeImageSnapshot</a>
     */
    @NotNull
    ...
    /**
     * <p>
     * Like the no-parameter version, this returns an image of the current surface
     * contents.
     * </p>
     *
     * <p>
     * This variant takes a rectangle specifying the subset of the surface that is
     * of interest.
     * These bounds will be sanitized before being used.
     * </p>
     *
     * <ul>
     * <li>If bounds extends beyond the surface, it will be trimmed to just the
     * intersection of it and the surface.</li>
     * <li>If bounds does not intersect the surface, then this returns null.</li>
     * <li>If bounds == the surface, then this is the same as calling the
     * no-parameter variant.</li>
     * </ul>
     *
     * @return Image initialized with Surface contents or null
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_makeImageSnapshot_2">https://fiddle.skia.org/c/@Surface_makeImageSnapshot_2</a>
     */
    @Nullable
     ...
     *
     * <p>
     * If Paint paint is not null, apply ColorFilter, alpha, ImageFilter, and
     * BlendMode.
     * </p>
     *
     ...
     * @param paint  Paint containing BlendMode, ColorFilter, ImageFilter, and so
     *               on; or null
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_draw">https://fiddle.skia.org/c/@Surface_draw</a>
     */
     ...
     * <li>Surface pixels could not be converted to dst.colorType() or
     * dst.alphaType().</li>
     ...
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_readPixels_3">https://fiddle.skia.org/c/@Surface_readPixels_3</a>
     */
     ...
     * Destination Rect corners are (dstX, dstY) and (dstX + Surface width(), dstY +
     * Surface height()).
     * </p>
     *
     * <p>
     ...
     * @param x      x-axis position relative to Surface to begin copy; may be
     *               negative
     * @param y      y-axis position relative to Surface to begin copy; may be
     *               negative
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Surface_writePixels_2">https://fiddle.skia.org/c/@Surface_writePixels_2</a>
     */
     ...
     * This must to be used to ensure correct ordering when the surface backing
     * store is accessed
     * outside Skia (e.g. direct use of the 3D API or a windowing system).
     * DirectContext has additional flush and submit methods that apply to all
     * surfaces and images created from
     * a DirectContext.
     */
    ...
    public static native long _nWrapPixels(int width, int height, int colorType, int alphaType, long colorSpacePtr,
            long pixelsPtr, long rowBytes, SurfaceProps surfaceProps);

    public static native long _nWrapPixelsPixmap(long pixmapPtr, SurfaceProps surfaceProps);

    public static native long _nMakeRaster(int width, int height, int colorType, int alphaType, long colorSpacePtr,
            long rowBytes, SurfaceProps surfaceProps);

    public static native long _nWrapBackendRenderTarget(long pContext, long pBackendRenderTarget, int surfaceOrigin,
            int colorType, long colorSpacePtr, SurfaceProps surfaceProps);

    public static native long _nWrapMTKView(long contextPtr, long mtkViewPtr, int surfaceOrigin, int sampleCount,
            int colorType, long colorSpacePtr, SurfaceProps surfaceProps);

    public static native long _nMakeRenderTarget(long contextPtr, boolean budgeted, int width, int height,
            int colorType, int alphaType, long colorSpacePtr, int sampleCount, int surfaceOrigin,
            SurfaceProps surfaceProps, boolean shouldCreateWithMips);
    ...
    public static native long _nMakeSurfaceI(long ptr, int width, int height, int colorType, int alphaType,
            long colorSpacePtr);
