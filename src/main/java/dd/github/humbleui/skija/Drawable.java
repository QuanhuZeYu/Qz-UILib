 ...
 * drawables. To
 * allow for clients of the drawable that may want to cache the results, the
 * drawable must
...
public abstract class Drawable extends RefCnt {
    ...
    /**
     * Draws into the specified content. The drawing sequence will be balanced upon
     * return
     * (i.e. the saveLevel() on the canvas will match what it was when draw() was
     * called,
    ...
    /**
     * <p>
     * Return a unique value for this instance. If two calls to this return the same
     * value,
     * it is presumed that calling the draw() method will render the same thing as
     * well.
     * </p>
     *
     * <p>
     * Subclasses that change their state should call notifyDrawingChanged() to
     * ensure that
    ...
    /**
     * Return the (conservative) bounds of what the drawable will draw. If the
     * drawable can
     * change what it draws (e.g. animation or in response to some external change),
     * then this
    ...
    /**
     * Calling this invalidates the previous generation ID, and causes a new one to
     * be computed
     * the next time getGenerationId() is called. Typically this is called by the
     * object itself,
