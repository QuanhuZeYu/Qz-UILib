...
public interface IHasImageInfo {
     ...
     *
     * May be less than getPixelRef().getWidth(). Will not exceed
     * getPixelRef().getWidth() less
     *
     ...
     *
     * Maybe be less than getPixelRef().getHeight(). Will not exceed
     * getPixelRef().getHeight()
     *
     ...
     *
     * Does not check if PixelRef is null; call {@link Bitmap#drawsNothing()} to
     * check
    ...
    /**
     * <p>
     * Returns true if ColorAlphaType is set to hint that all pixels are opaque;
     * their
