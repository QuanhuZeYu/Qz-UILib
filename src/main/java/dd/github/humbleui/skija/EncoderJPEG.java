...
public class EncoderJPEG {
     ...
     *
     * @param ctx   If the image was created as a texture-backed image on a GPU
     *              context,
     *              that ctx must be provided so the pixels can be read before being
     *              encoded.
    ...
    public static Data encode(@Nullable DirectContext ctx, @NotNull Image image, @NotNull EncodeJPEGOptions opts) {
        try {
            ...
            Stats.onNativeCall();
            long ptr = _nEncode(Native.getPtr(ctx), Native.getPtr(image), opts._quality, opts._downsampleMode.ordinal(),
                    opts._alphaMode.ordinal());
    ...
    @ApiStatus.Internal
    public static native long _nEncode(long ctxPtr, long imagePtr, int quality, int downsampleMode, int alphaMode);
}
