...
public class TextBlob extends Managed {
     ...
     * The return array size is a multiple of two, and is at most twice the number
     * of glyphs in
     * the the blob.
     * </p>
     *
     ...
     * @param lowerBound lower line parallel to the advance
     * @param upperBound upper line parallel to the advance
     * @param paint      specifies stroking, PathEffect that affects the result; may
     *                   be null
     * @return intersections; may be null
     */
    @Nullable
    ...
    /**
     * Returns a TextBlob built from a single run of text with x-positions and a
     * single y value.
     * Returns null if glyphs is empty.
     *
     * @param glyphs glyphs drawn
     * @param xpos   array of x-positions, must contain values for all of the
     *               glyphs.
    ...
    public static TextBlob makeFromRSXform(short[] glyphs, RSXform[] xform, Font font) {
        try {
            assert glyphs.length == xform.length
                    : "glyphs.length " + glyphs.length + " != xform.length " + xform.length;
     ...
     *
     * <ul>
     * <li>makeFromPosH returns 1 float per glyph (x pos)
     * <li>makeFromPos returns 2 floats per glyph (x, y pos)
     * <li>makeFromRSXform returns 4 floats per glyph
     * </ul>
     *
     * <p>
     * Blobs constructed by TextBlobBuilderRunHandler/Shaper default handler have 2
     * floats per glyph.
     * </p>
     *
     * @return glyph positions for the blob if it was made with makeFromPos, null
     *         otherwise
     */
    @NotNull
    ...
    /**
     * Only works on TextBlobs that come from TextBlobBuilderRunHandler/Shaper
     * default handler.
     *
    ...
    @ApiStatus.Internal
    public static native float[] _nGetIntercepts(long ptr, float lower, float upper, long paintPtr);

    @ApiStatus.Internal
    public static native long _nMakeFromPosH(short[] glyphs, float[] xpos, float ypos, long fontPtr);

    @ApiStatus.Internal
    public static native long _nMakeFromPos(short[] glyphs, float[] pos, long fontPtr);

    @ApiStatus.Internal
    public static native long _nMakeFromRSXform(short[] glyphs, float[] xform, long fontPtr);
    ...
    @ApiStatus.Internal
    public static native int[] _nGetClusters(long ptr);

    @ApiStatus.Internal
    public static native Rect _nGetTightBounds(long ptr);

    @ApiStatus.Internal
    public static native Rect _nGetBlockBounds(long ptr);

    @ApiStatus.Internal
    public static native Float _nGetFirstBaseline(long ptr);

    @ApiStatus.Internal
    public static native Float _nGetLastBaseline(long ptr);
}
