...
public class ColorSpace extends Managed {
    ...
    public Color4f convert(ColorSpace to, Color4f color) {
        ...
        try {
            return new Color4f(
                    _nConvert(_ptr, Native.getPtr(to), color.getR(), color.getG(), color.getB(), color.getA()));
        } finally {
    ...
    /**
     * @return true if the color space gamma is near enough to be approximated as
     *         sRGB
     */
     ...
     * numerical error
     * in some cases: converting ICC fixed point to float, converting white point to
     * D50,
     ...
     * these
     * functions are similar (and it is sometimes useful to consider them together),
     * this
