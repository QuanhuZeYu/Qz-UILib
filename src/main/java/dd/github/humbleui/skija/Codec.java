...
public class Codec extends Managed implements IHasImageInfo {
     ...
     * return an Codec that can decode it. Otherwise throws
     ...
     * </p>
     *
     * @param bitmap the description of the format (config, size) expected by the
     *               caller
     * @return this
     */
    @NotNull
     ...
     * </p>
     *
     * @param bitmap     the description of the format (config, size) expected by
     *                   the caller
     * @param frame      index of the frame in multi-frame image to decode
     * @param priorFrame index of the frame already in bitmap, might be used to
     *                   optimize retrieving current frame
     * @return this
     */
    @NotNull
    @Contract("_ -> this")
    ...
    /**
     * <p>
     * Return the number of times to repeat, if this image is animated. This number
     * does not
     * include the first play through of each frame. For example, a repetition count
     * of 4 means
    ...
    public static void _validateResult(int result) {
        switch (result) {
            ...
            case 5: // kInvalidParameters
                throw new IllegalArgumentException(
                        "Invalid parameter: Parameters (besides info) are invalid. e.g. NULL pixels, rowBytes too small, etc");
            ...
            case 7: // kCouldNotRewind
                throw new UnsupportedOperationException(
                        "Could not rewind: Fulfilling this request requires rewinding the input, which is not supported for this input");
    ...
    @ApiStatus.Internal
    public static native long _nGetSize(long ptr);

    @ApiStatus.Internal
    public static native int _nGetEncodedOrigin(long ptr);

    @ApiStatus.Internal
    public static native int _nGetEncodedImageFormat(long ptr);

    @ApiStatus.Internal
    public static native int _nReadPixels(long ptr, long bitmapPtr, int frame, int priorFrame);

    @ApiStatus.Internal
    public static native int _nGetFrameCount(long ptr);

    @ApiStatus.Internal
    public static native AnimationFrameInfo _nGetFrameInfo(long ptr, int frame);

    @ApiStatus.Internal
    public static native AnimationFrameInfo[] _nGetFramesInfo(long ptr);

    @ApiStatus.Internal
    public static native int _nGetRepetitionCount(long ptr);
}
