...
public class Canvas extends Managed {
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_copy_const_SkBitmap">https://fiddle.skia.org/c/@Canvas_copy_const_SkBitmap</a>
     */
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_const_SkBitmap_const_SkSurfaceProps">https://fiddle.skia.org/c/@Canvas_const_SkBitmap_const_SkSurfaceProps</a>
     */
    public Canvas(@NotNull Bitmap bitmap, @NotNull SurfaceProps surfaceProps) {
        this(_nMakeFromBitmap(bitmap._ptr, surfaceProps._getFlags(), surfaceProps._pixelGeometry.ordinal()), true,
                bitmap);
        Stats.onNativeCall();
    ...
    /**
     * Returns the SurfaceProps associated with the canvas (i.e., at the base of the
     * layer
     * stack).
     *
     * @return SurfaceProps
     ...
     * (i.e., at
     * the top of the layer stack). This can differ from {getBaseProps()} depending
     * on the flags
     * passed to saveLayer.
     *
     ...
     * PaintStrokeCap. If paint is set to {@link PaintStrokeCap#ROUND}, each point
     * draws a
     * circle of diameter Paint stroke width. If paint is set to
     * {@link PaintStrokeCap#SQUARE}
     ...
     * PaintMode is ignored, as if were set to {@link PaintMode#STROKE}.
     * </p>
     *
     * <p>
     * Always draws each element one at a time; is not affected by
     * PaintStrokeJoin, and unlike drawPath(), does not create a mask from all
     * points
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_drawPoints">https://fiddle.skia.org/c/@Canvas_drawPoints</a>
     */
    @NotNull
    @Contract("_, _ -> this")
     ...
     * One line is drawn for every two points; each point is used once. If count is
     * odd,
    ...
    public Canvas drawArc(float left, float top, float right, float bottom, float startAngle, float sweepAngle,
            boolean includeCenter, @NotNull Paint paint) {
        assert _ptr != 0 : "Canvas is closed";
    ...
    public Canvas drawDRRect(@NotNull RRect outer, @NotNull RRect inner, @NotNull Paint paint) {
        ...
        Stats.onNativeCall();
        _nDrawDRRect(_ptr, outer._left, outer._top, outer._right, outer._bottom, outer._radii, inner._left, inner._top,
                inner._right, inner._bottom, inner._radii, Native.getPtr(paint));
        ReferenceUtil.reachabilityFence(paint);
        return this;
    }
    ...
    public Canvas drawImage(@NotNull Image image, float left, float top) {
        return drawImageRect(image, Rect.makeWH(image.getWidth(), image.getHeight()),
                Rect.makeXYWH(left, top, image.getWidth(), image.getHeight()), SamplingMode.DEFAULT, null, true);
    }
    ...
    public Canvas drawImage(@NotNull Image image, float left, float top, @Nullable Paint paint) {
        return drawImageRect(image, Rect.makeWH(image.getWidth(), image.getHeight()),
                Rect.makeXYWH(left, top, image.getWidth(), image.getHeight()), SamplingMode.DEFAULT, paint, true);
    }

    @NotNull
    @Contract("_, _ -> this")
    public Canvas drawImageRect(@NotNull Image image, @NotNull Rect dst) {
        return drawImageRect(image, Rect.makeWH(image.getWidth(), image.getHeight()), dst, SamplingMode.DEFAULT, null,
                true);
    }
    ...
    public Canvas drawImageRect(@NotNull Image image, @NotNull Rect dst, @Nullable Paint paint) {
        return drawImageRect(image, Rect.makeWH(image.getWidth(), image.getHeight()), dst, SamplingMode.DEFAULT, paint,
                true);
    }
    ...
    @Contract("_, _, _, _, _ -> this")
    public Canvas drawImageRect(@NotNull Image image, @NotNull Rect src, @NotNull Rect dst,
            @NotNull SamplingMode samplingMode, @Nullable Paint paint, boolean strict) {
        assert _ptr != 0 : "Canvas is closed";
        ...
        Stats.onNativeCall();
        _nDrawImageRect(_ptr, Native.getPtr(image), src._left, src._top, src._right, src._bottom, dst._left, dst._top,
                dst._right, dst._bottom, samplingMode._pack(), Native.getPtr(paint), strict);
    ...
    @NotNull
    @Contract("_, _, _, _, _ -> this")
    public Canvas drawImageNine(@NotNull Image image, @NotNull IRect center, @NotNull Rect dst,
            @NotNull FilterMode filterMode, @Nullable Paint paint) {
        assert _ptr != 0 : "Canvas is closed";
        ...
        Stats.onNativeCall();
        _nDrawImageNine(_ptr, Native.getPtr(image), center._left, center._top, center._right, center._bottom, dst._left,
                dst._top, dst._right, dst._bottom, filterMode.ordinal(), Native.getPtr(paint));
        ReferenceUtil.reachabilityFence(image);
        ReferenceUtil.reachabilityFence(paint);
        return this;
    }
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_drawVertices">https://fiddle.skia.org/c/@Canvas_drawVertices</a>
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_drawVertices_2">https://fiddle.skia.org/c/@Canvas_drawVertices_2</a>
     */
    @NotNull
    @Contract("_, _, _ -> this")
     ...
     *
     * <p>
     * If paint contains an Shader and vertices does not contain texCoords, the
     * shader
     ...
     * @param positions triangle mesh to draw
     * @param colors    color array, one for each corner; may be null
     * @param texCoords Point array of texture coordinates, mapping Shader to
     *                  corners; may be null
    ...
    @NotNull
    @Contract("_, _, _, _, _ -> this")
    public Canvas drawTriangles(@NotNull Point[] positions, @Nullable int[] colors, @Nullable Point[] texCoords,
            @Nullable short[] indices, @NotNull Paint paint) {
    ...
    public Canvas drawTriangles(@NotNull Point[] positions, @Nullable int[] colors, @Nullable Point[] texCoords,
            @Nullable short[] indices, @NotNull BlendMode mode, @NotNull Paint paint) {
        assert _ptr != 0 : "Canvas is closed";
        ...
        assert colors == null || colors.length == positions.length
                : "Expected colors.length == positions.length, got: " + colors.length + " != " + positions.length;
        assert texCoords == null || texCoords.length == positions.length
                : "Expected texCoords.length == positions.length, got: " + texCoords.length + " != " + positions.length;
        assert paint != null : "Can’t drawTriangles with paint == null";
        Stats.onNativeCall();
        _nDrawVertices(_ptr, 0 /* kTriangles_VertexMode */, Point.flattenArray(positions), colors,
                Point.flattenArray(texCoords), indices, mode.ordinal(), Native.getPtr(paint));
        ReferenceUtil.reachabilityFence(paint);
        return this;
    }
    ...
    @NotNull
    @Contract("_, _, _, _, _ -> this")
    public Canvas drawTriangleStrip(@NotNull Point[] positions, @Nullable int[] colors, @Nullable Point[] texCoords,
            @Nullable short[] indices, @NotNull Paint paint) {
    ...
    public Canvas drawTriangleStrip(@NotNull Point[] positions, @Nullable int[] colors, @Nullable Point[] texCoords,
        ...
        assert paint != null : "Can’t drawTriangles with paint == null";
        Stats.onNativeCall();
        _nDrawVertices(_ptr, 1 /* kTriangleStrip_VertexMode */, Point.flattenArray(positions), colors,
                Point.flattenArray(texCoords), indices, mode.ordinal(), Native.getPtr(paint));
        ReferenceUtil.reachabilityFence(paint);
        return this;
    }
    ...
    @NotNull
    @Contract("_, _, _, _, _ -> this")
    public Canvas drawTriangleFan(@NotNull Point[] positions, @Nullable int[] colors, @Nullable Point[] texCoords,
            @Nullable short[] indices, @NotNull Paint paint) {
    ...
    public Canvas drawTriangleFan(@NotNull Point[] positions, @Nullable int[] colors, @Nullable Point[] texCoords,
        ...
        Stats.onNativeCall();
        _nDrawVertices(_ptr, 2 /* kTriangleFan_VertexMode */, Point.flattenArray(positions), colors,
                Point.flattenArray(texCoords), indices, mode.ordinal(), Native.getPtr(paint));
        ReferenceUtil.reachabilityFence(paint);
        return this;
    }
     ...
     * in clockwise order, sharing every fourth point. The last Path cubic ends at
     * the
     * first point.
     * </p>
     *
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/4cf70f8d194867d053d7e177e5088445">https://fiddle.skia.org/c/4cf70f8d194867d053d7e177e5088445</a>
     */
    @NotNull
    @Contract("_, _, _ -> this")
     ...
     * corners in top-left, top-right, bottom-right, bottom-left order. If texCoords
     * is
    ...
    @NotNull
    @Contract("_, _, _, _ -> this")
    public Canvas drawPatch(@NotNull Point[] cubics, @NotNull int[] colors, @Nullable Point[] texCoords,
            @NotNull Paint paint) {
    ...
    @Contract("_, _, _, _, _ -> this")
    public Canvas drawPatch(@NotNull Point[] cubics, @NotNull int[] colors, @Nullable Point[] texCoords,
            @NotNull BlendMode mode, @NotNull Paint paint) {
        assert _ptr != 0 : "Canvas is closed";
        ...
        Stats.onNativeCall();
        _nDrawPatch(_ptr, Point.flattenArray(cubics), colors, Point.flattenArray(texCoords), mode.ordinal(),
                Native.getPtr(paint));
        ReferenceUtil.reachabilityFence(paint);
        return this;
    }
     ...
     * so that Drawable::draw() can be called when the operation is finalized. To
     * force
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_drawDrawable_2">https://fiddle.skia.org/c/@Canvas_drawDrawable_2</a>
     */
    @NotNull
    @Contract("_, _, _ -> this")
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_drawDrawable">https://fiddle.skia.org/c/@Canvas_drawDrawable</a>
     */
    @NotNull
    @Contract("_, _ -> this")
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_setMatrix">https://fiddle.skia.org/c/@Canvas_setMatrix</a>
     */
    @NotNull
    @Contract("_ -> this")
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_resetMatrix">https://fiddle.skia.org/c/@Canvas_resetMatrix</a>
     */
    @NotNull
    @Contract("-> this")
     ...
     *
     * <p>
     * Source Rect corners are (srcX, srcY) and (imageInfo().width(),
     * imageInfo().height()).
     ...
     * returned by PictureRecorder::beginRecording, or Canvas is the base of a
     * utility
     ...
     * <li>SkCanvas pixels could not be converted to bitmap.colorType() or
     * bitmap.alphaType().</li>
     * <li>SkCanvas pixels are not readable; for instance, Canvas is
     * document-based.</li>
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_readPixels_3">https://fiddle.skia.org/c/@Canvas_readPixels_3</a>
     */
     ...
     * <p>
     * Copies each readable pixel intersecting both rectangles, without scaling,
     * converting to getImageInfo().getColorType() and getImageInfo().getAlphaType()
     * if required.
     * </p>
     *
     * <p>
     ...
     * <li>bitmap pixels could not be converted to Canvas
     * getImageInfo().getColorType() or
     * getImageInfo().getAlphaType().</li>
     * <li>Canvas pixels are not writable; for instance, Canvas is document
     * based.</li>
     * <li>bitmap pixels are inaccessible; for instance, bitmap wraps a
     * texture.</li>
     * </ul>
     *
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_writePixels_2">https://fiddle.skia.org/c/@Canvas_writePixels_2</a>
     * @see <a href=
     *      "https://fiddle.skia.org/c/@State_Stack_a">https://fiddle.skia.org/c/@State_Stack_a</a>
     * @see <a href=
     *      "https://fiddle.skia.org/c/@State_Stack_b">https://fiddle.skia.org/c/@State_Stack_b</a>
     */
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_saveLayer">https://fiddle.skia.org/c/@Canvas_saveLayer</a>
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_saveLayer_4">https://fiddle.skia.org/c/@Canvas_saveLayer_4</a>
     */
    public int saveLayer(@Nullable Rect bounds, @Nullable Paint paint) {
        try {
            ...
            if (bounds == null)
                ...
                return _nSaveLayerRect(_ptr, bounds._left, bounds._top, bounds._right, bounds._bottom,
                        Native.getPtr(paint));
        } finally {
            ReferenceUtil.reachabilityFence(this);
            ReferenceUtil.reachabilityFence(paint);
     ...
     *
     * <p>
     * Bounds suggests but does not define layer size. To clip drawing to
     * a specific rectangle, use clipRect().
     * </p>
     *
     * <p>
     ...
     * @param alpha  opacity of layer, zero is fully transparent, 255 is fully
     *               opaque.
     * @return depth of saved stack
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_saveLayerAlpha">https://fiddle.skia.org/c/@Canvas_saveLayerAlpha</a>
     */
     ...
     *
     * @see <a href=
     *      "https://fiddle.skia.org/c/@Canvas_saveLayer_3">https://fiddle.skia.org/c/@Canvas_saveLayer_3</a>
     */
    public int saveLayer(@NotNull SaveLayerRec layerRec) {
        try {
            ...
            if (bounds == null)
                return _nSaveLayerRec(_ptr, Native.getPtr(layerRec._paint), Native.getPtr(layerRec._backdrop),
                        layerRec._flags);
            else
                return _nSaveLayerRecRect(_ptr, bounds._left, bounds._top, bounds._right, bounds._bottom,
                        Native.getPtr(layerRec._paint), Native.getPtr(layerRec._backdrop), layerRec._flags);
        } finally {
            ReferenceUtil.reachabilityFence(this);
    ...
    public static native void _nDrawArc(long ptr, float left, float top, float right, float bottom, float startAngle,
            float sweepAngle, boolean includeCenter, long paintPtr);
    ...
    public static native void _nDrawRRect(long ptr, float left, float top, float right, float bottom, float[] radii,
            long paintPtr);
    ...
    public static native void _nDrawDRRect(long ptr, float ol, float ot, float or, float ob, float[] oradii, float il,
            float it, float ir, float ib, float[] iradii, long paintPtr);

    public static native void _nDrawPath(long ptr, long nativePath, long paintPtr);

    public static native void _nDrawImageRect(long ptr, long nativeImage, float sl, float st, float sr, float sb,
            float dl, float dt, float dr, float db, long samplingMode, long paintPtr, boolean strict);

    public static native void _nDrawImageNine(long ptr, long nativeImage, int cl, int ct, int cr, int cb, float dl,
            float dt, float dr, float db, int filterMode, long paintPtr);
    ...
    public static native void _nDrawVertices(long ptr, int verticesMode, float[] cubics, int[] colors,
            float[] texCoords, short[] indices, int blendMode, long paintPtr);

    public static native void _nDrawPatch(long ptr, float[] cubics, int[] colors, float[] texCoords, int blendMode,
            long paintPtr);
    ...
    public static native void _nClipRect(long ptr, float left, float top, float right, float bottom, int mode,
            boolean antiAlias);

    public static native void _nClipRRect(long ptr, float left, float top, float right, float bottom, float[] radii,
            int mode, boolean antiAlias);
    ...
    public static native int _nSaveLayerAlphaRect(long ptr, float left, float top, float right, float bottom,
            int alpha);

    public static native int _nSaveLayerRec(long ptr, long paintPtr, long backdropPtr, int flags);

    public static native int _nSaveLayerRecRect(long ptr, float left, float top, float right, float bottom,
            long paintPtr, long backdropPtr, int flags);
