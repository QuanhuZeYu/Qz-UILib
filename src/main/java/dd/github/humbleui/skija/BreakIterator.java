 ...
 * <li>makeWordInstance() returns a BreakIterator that locates boundaries
 * between
 ...
 * <li>makeLineInstance() returns a BreakIterator that locates positions where
 * it is
 ...
 * don't
 * want a line to start with whitespace, for example), and some special
 * characters
 * can force a position to be considered a line-break position or prevent a
 * position
 * from being a line-break position.
 * <li>makeCharacterInstance() returns a BreakIterator that locates boundaries
 * between
 * logical characters. Because of the structure of the Unicode encoding, a
 * logical
 * character may be stored internally as more than one Unicode code point. (A
 * with an
 * umlaut may be stored as an a followed by a separate combining umlaut
 * character,
 * for example, but the user still thinks of it as one character.) This iterator
 * allows
 * various processes (especially text editors) to treat as characters the units
 * of text
 * that a user would think of as characters, rather than the units of text that
 * the
 ...
 * meaning it
 * has a concept of a "current position" and methods like first(), last(),
 * next(),
 * and previous() that update the current position. All BreakIterators uphold
 * the
 ...
 * follow
 * them. Thus, under normal circumstances, the position before the first
 * character
 ...
 * analyzes,
 * at will, but cannot change the behavior. If the user wants different
 * behavior, he
 ...
 * which makes
 * it possible to use BreakIterator to analyze text in any text-storage vehicle
 * that
 ...
 * document
 * (or whatever stretch of text you're wrapping) and use it to do the whole job
 * of
 * wrapping the text.
 *
 * <P>
 ...
 * above code uses {@link #getRuleStatus()} to identify and ignore boundaries
 * associated
...
public class BreakIterator extends Managed implements Cloneable {
    ...
    /**
     * Returns a new BreakIterator instance for character breaks for the default
     * locale.
     */
    ...
    /**
     * Returns a new BreakIterator instance for character breaks for the given
     * locale.
     */
    ...
    /**
     * Returns a new BreakIterator instance for sentence breaks for the default
     * locale.
     */
    ...
    /**
     * Returns a new BreakIterator instance for sentence breaks for the given
     * locale.
     */
    ...
    /**
     * Returns the last boundary. The iterator's current position is set to the last
     * text boundary.
     */
    public int last() {
        try {
    ...
    @ApiStatus.Internal
    public static native long _nMake(int type, String locale);

    @ApiStatus.Internal
    public static native long _nClone(long ptr);

    @ApiStatus.Internal
    public static native int _nCurrent(long ptr);

    @ApiStatus.Internal
    public static native int _nNext(long ptr);

    @ApiStatus.Internal
    public static native int _nPrevious(long ptr);

    @ApiStatus.Internal
    public static native int _nFirst(long ptr);

    @ApiStatus.Internal
    public static native int _nLast(long ptr);

    @ApiStatus.Internal
    public static native int _nPreceding(long ptr, int offset);

    @ApiStatus.Internal
    public static native int _nFollowing(long ptr, int offset);

    @ApiStatus.Internal
    public static native boolean _nIsBoundary(long ptr, int offset);

    @ApiStatus.Internal
    public static native int _nGetRuleStatus(long ptr);

    @ApiStatus.Internal
    public static native int[] _nGetRuleStatuses(long ptr);

    @ApiStatus.Internal
    public static native void _nSetText(long ptr, long textPtr);
}
