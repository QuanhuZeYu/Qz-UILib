 ...
 * improve performance. Matrix is not thread safe unless getType() is called
 * first.
 * </p>
 *
 * @see <a href=
 *      "https://fiddle.skia.org/c/@Matrix_063">https://fiddle.skia.org/c/@Matrix_063</a>
 */
public class Matrix33 {
   ...
   public Matrix33 makePreScale(float sx, float sy) {
      return new Matrix33(new float[] { _mat[0] * sx, _mat[1] * sy, _mat[2], _mat[3] * sx, _mat[4] * sy, _mat[5],
            _mat[6] * sx, _mat[7] * sy, _mat[8] });
   }

   /**
    * <p>
    * Creates Matrix33 by multiplying this by other. This can be thought of mapping
    * by other before applying Matrix.
    * </p>
    *
    * <p>
    * Given:
   ...
   public Matrix33 makeConcat(Matrix33 other) {
      return new Matrix33(new float[] { _mat[0] * other._mat[0] + _mat[1] * other._mat[3] + _mat[2] * other._mat[6],
   ...
   /**
    * Creates a Matrix33 to rotate by |deg| about a pivot point at (pivotx,
    * pivoty).
    *
    * @param deg    rotation angle in degrees (positive rotates clockwise)
   ...
   public static Matrix33 makeRotate(float deg, float pivotx, float pivoty) {
      ...
      if (Math.abs(cos) <= tolerance)
         cos = 0;
      return new Matrix33(new float[] { (float) cos, (float) -sin, (float) (pivotx - pivotx * cos + pivoty * sin),
            (float) sin, (float) cos, (float) (pivoty - pivoty * cos - pivotx * sin), 0, 0, 1 });
   }
   ...
   public Matrix44 asMatrix44() {
      return new Matrix44(_mat[0], _mat[1], 0, _mat[2], _mat[3], _mat[4], 0, _mat[5], 0, 0, 1, 0, _mat[6], _mat[7], 0,
            _mat[8]);
   }
