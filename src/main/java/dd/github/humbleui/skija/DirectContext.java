...
public class DirectContext extends RefCnt {
     ...
     * </p>
     * <p>
     * For more information refer to skia GrDirectContext::submit(bool syncCpu)
     * method.
     * </p>
     *
     ...
     * is no longer
     * usable. Call this if you have lost the associated GPU context, and thus
     * internal texture,
     * buffer, etc. references/IDs are now invalid. Calling this ensures that the
     * destructors of the
     * context and any of its created resource objects will not make backend 3D API
     * calls. Content
     ...
     * VkInstance used to
     * create the context must be kept alive even after abandoning the context.
     * Those objects must
     * live for the lifetime of the context object itself. The reason for this is so
     * that
     * we can continue to delete any outstanding GrBackendTextures/RenderTargets
     * which must be
    ...
    @ApiStatus.Internal
    public static native long _nMakeMetal(long devicePtr, long queuePtr);

    @ApiStatus.Internal
    public static native long _nMakeDirect3D(long adapterPtr, long devicePtr, long queuePtr);

    @ApiStatus.Internal
    public static native long _nFlush(long ptr);

    @ApiStatus.Internal
    public static native long _nSubmit(long ptr, boolean syncCpu);
