...
| 2026-06-20 | scene 布局引擎 COLUMN 容器**主轴**（高度）方向的 fill 子节点未实现（含单子），子回退 shrink-to-fit | 深层 fill 约束下传还清时（上一条方案甲），ROW 交叉轴高可多子共享同一下传高、无冲突；COLUMN 主轴高多个 fill 子需按比例分配父高，必须引入 flex-grow 求解器才能正确分配，超出本期范围。COLUMN 单子 fill 主轴理论无分配冲突本可支持，但当前 `buildChildConstraints` 对 COLUMN 一刀切传 `UNCONSTRAINED`（安全方向过度收窄），未单独放行单子。属有意 YAGNI。 | scene 层 `SceneLayoutEngine.buildChildConstraints`（COLUMN 分支恒 UNCONSTRAINED）；影响「COLUMN 容器内子节点需填满父高」的未来场景。当前正确性无损（子 shrink 不溢出，反证锚点 `columnFillChildrenDoNotOverflowParent` 固化）。已在 `SceneNode.setFillParentHeight` javadoc 写明此边界，防后续 agent 当 bug 误修。 | 回填方向：出现真实「COLUMN 多 fill 子按比例分高」需求时引入 flex-grow 求解器（主轴剩余空间按 grow 权重分配）；单子 fill 主轴可作为前置小步先放行（`buildChildConstraints` COLUMN 分支在 `children.size()==1 && 高先验确定` 时下传主轴高）。优先级：待真实需求启动，须经 oracle 评估分配语义对 I7 的影响。回归锚点：`columnFillChildrenDoNotOverflowParent`（届时扩单子 fill 断言）。 |
