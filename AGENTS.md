...
- **不变量只写语义，不写编号**：旧 `I1..I13`、`信条一~八`、`偏离登记` 的编号体系已整体废止，`NORTH_STAR.md` 已删、编号出处不存在；`src/` 内的编号引用已于 2026-09-14 全仓就地改写清零（含 `信条`/`宪章`、连字符子项号 `I-1..I-6` 与已删文件指针）。这些编号命名的约束本身**仍是活规矩**（由现存测试钉着），但**新写注释、javadoc 与文档一律不得再引任何外部编号**——要引就把它要守的约束本身写进注释：「数据层不 import 渲染/样式层」「干净子树在布局/绘制/合成三阶段被整棵跳过」（`SceneLayoutEngine` 的 cachedLayout 短路即其实现）「平台类型止于适配边界」（`PlatformInputSource`）「分级失效四级模型」（`Invalidation` 现行定义）。现存红线的权威是**所在处注释与测试**；历史提交里的编号只作追溯线索（旧表可由 `git show fca36d13^:NORTH_STAR.md` 取回），不再是契约标识。测试与评审同样按语义点名（如「`ScenePackageIsolationTest`/`LayerContractGuardTest` 守数据层不 import 渲染/样式层」），不写「守 I6」。
...
- **任务体系票号是出处不是约束**：`C#`/`F#`/`U#`/`B#`/`P#`/`D#`/`S#`/`G#`/`T#`、`§小节号`、`第 N 条`（规划/核准表条目号，如 `R2`、`R3-C`、`§4.5`）指向已删任务与规划体系；新写注释不带票号，存量碰到时就地降级为「做了什么」的直述，不再当规则引用。存量规模：注释内票号形引用约 4000 处 / 500 文件、`§\d` 约 870 处 / 205 文件，属后续批次（机会主义收敛，不阻塞当前工作）。**清理只改注释文本：不动代码标识符、字符串字面量、断言表达式**。**同形活编号不得误伤**：`R1-R13` 是控件契约红线（定义处 `ui/scene/control/package-info.java`，属现行规范）、`L1/L2` 是文档/解析层名、`GL\d+` 是 GL 常量——三类不进清理清单。
